(function() {
  ...
  // build/amp-story-entry-point.css.js
  var cssText = "amp-story-entry-point{position:relative;display:block}\n/*# sourceURL=/css/amp-story-entry-point.css*/";

  // src/amp-story-player/amp-story-entry-point/amp-story-entry-point-impl.js
  var AmpStoryEntryPoint = /* @__PURE__ */ function() {
    function AmpStoryEntryPoint2(win, element) {
      this.win_ = win;
    ...
    var _proto = AmpStoryEntryPoint2.prototype;
  ...
  var AmpStoryComponentManager = /* @__PURE__ */ function() {
    function AmpStoryComponentManager2(win) {
      this.win_ = win;
    }
    var _proto = AmpStoryComponentManager2.prototype;
    _proto.layoutEl_ = function layoutEl_(element) {
      var _this = this;
      new AmpStoryPlayerViewportObserver(this.win_, element, function() {
        return element.layoutPlayer();
      });
      var scrollHandler = function scrollHandler2() {
        element.layoutPlayer();
        _this.win_.removeEventListener("scroll", scrollHandler2);
      };
      this.win_.addEventListener("scroll", scrollHandler);
    };
    _proto.loadPlayers = function loadPlayers() {
      var doc = this.win_.document;
      var players = doc.getElementsByTagName("amp-story-player");
      initLogConstructor();
      for (var i = 0; i < players.length; i++) {
        var playerEl = players[i];
        var player = new AmpStoryPlayer(this.win_, playerEl);
        player.buildPlayer();
        this.layoutEl_(player);
      }
    };
    _proto.loadEntryPoints = function loadEntryPoints() {
      var doc = this.win_.document;
      var entryPoints = doc.getElementsByTagName("amp-story-entry-point");
      initLogConstructor();
      for (var i = 0; i < entryPoints.length; i++) {
        var entryPointEl = entryPoints[i];
        var entryPoint = new AmpStoryEntryPoint(this.win_, entryPointEl);
        entryPoint.buildCallback();
        this.layoutEl_(entryPoint);
      }
    };
    return AmpStoryComponentManager2;
  }();

  // src/amp-story-player/amp-story-player.js
  onDocumentReady(self.document, function() {
    var manager = new AmpStoryComponentManager(self);
    manager.loadPlayers();
  });
  globalThis.AmpStoryPlayer = AmpStoryPlayer;
})();
/*! https://mths.be/cssescape v1.5.1 by @mathias | MIT license */
//# sourceMappingURL=amp-story-player.js.map
