(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-connatix-player",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  var AmpConnatixPlayer = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpConnatixPlayer2, _AMP$BaseElement);
    var _super = _createSuper(AmpConnatixPlayer2);
    function AmpConnatixPlayer2(element) {
      var _this;
      _this = _super.call(this, element);
      _this.playerId_ = "";
      _this.mediaId_ = "";
      _this.iframeDomain_ = null;
      _this.iframe_ = null;
    ...
    var _proto = AmpConnatixPlayer2.prototype;
      ...
      this.playerReadyPromise_.then(function(iframe) {
        ...
        if (iframe.contentWindow) {
          iframe.contentWindow.postMessage(JSON.stringify(dict({
            ...
            "func": command,
            "args": opt_args || ""
          })), _this2.iframeDomain_);
        }
      });
    };
    _proto.bindToPlayerCommands_ = function bindToPlayerCommands_() {
      var _this3 = this;
      this.win.addEventListener("message", function(e) {
        if (!_this3.iframe_ || e.source !== _this3.iframe_.contentWindow) {
          return;
        }
        var dataString = getData(e);
        var dataJSON = tryParseJson(dataString);
        if (!dataJSON || dataJSON["event"] !== "command") {
          return;
        }
        switch (dataJSON["func"]) {
          case "cnxClose": {
            _this3.destroyPlayerFrame_();
            _this3.attemptCollapse();
            break;
          }
          case "cnxPlayerRendered": {
            _this3.playerReadyResolver_(_this3.iframe_);
            break;
          }
        }
      });
    };
    _proto.bindToAmpConsent_ = function bindToAmpConsent_() {
      var _this4 = this;
      ...
      var consentPolicyInfoPromise = getConsentPolicyInfo(this.element, consentPolicyId);
      var consentPolicySharedDataPromise = getConsentPolicySharedData(this.element, consentPolicyId);
      var consentMetadataPromise = getConsentMetadata(this.element, consentPolicyId);
      allSettled([consentPolicyStatePromise, consentPolicyInfoPromise, consentPolicySharedDataPromise, consentMetadataPromise]).then(function(values) {
        if (values && values.length === 4) {
          var consentPolicyState = values[0];
          var consentPolicyInfo = values[1];
          var consentPolicySharedData = values[2];
          var consentMetadata = values[3];
          var ampConsentInfo = {
          ...
          _this4.sendCommand_("ampConsentInfo", ampConsentInfo);
        }
      });
    };
    _proto.destroyPlayerFrame_ = function destroyPlayerFrame_() {
      if (this.iframe_) {
    ...
    _proto.buildCallback = function buildCallback() {
      ...
      this.playerId_ = userAssert2(element.getAttribute("data-player-id"), "The data-player-id attribute is required for <amp-connatix-player> %s", element);
      this.mediaId_ = element.getAttribute("data-media-id") || "";
      var elementsPlayer = element.getAttribute("data-elements-player") || false;
      if (elementsPlayer) {
    ...
    _proto.layoutCallback = function layoutCallback() {
      ...
      var element = this.element;
      var urlParams = dict({
        "playerId": this.playerId_ || void 0,
        "mediaId": this.mediaId_ || void 0,
        "url": Services.documentInfoForDoc(element).sourceUrl
      });
      var iframeUrl = this.iframeDomain_ + "/amp-embed/index.html";
      var src = addParamsToUrl(iframeUrl, urlParams);
      ...
      return this.loadPromise(iframe).then(function() {
        return _this5.playerReadyPromise_;
      });
    };
    _proto.isLayoutSupported = function isLayoutSupported(layout) {
      return isLayoutSizeDefined(layout);
    };
    _proto.onResized_ = function onResized_(_ref) {
      var height = _ref.height, width = _ref.width;
      if (!this.iframe_) {
        return;
      }
      this.sendCommand_("ampResize", {
        "width": width,
        "height": height
      });
    };
    ...
    return AmpConnatixPlayer2;
  }(AMP.BaseElement);
  AMP.registerElement("amp-connatix-player", AmpConnatixPlayer);
})();
/*! https://mths.be/cssescape v1.5.1 by @mathias | MIT license */
})});
//# sourceMappingURL=amp-connatix-player-0.1.max.js.map
