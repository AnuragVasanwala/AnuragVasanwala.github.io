(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-ima-video",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  var ImaPlayerData = /* @__PURE__ */ function() {
    ...
    var _proto = ImaPlayerData2.prototype;
    _proto.update = function update(videoPlayer) {
  ...
  var TAG4 = "amp-ima-video";
  var TYPE = "ima-video";
  function serializeAttributes(element) {
    var attributes = element.attributes;
    var serialized = {};
    for (var i = 0; i < attributes.length; i++) {
      var _attributes$i = attributes[i], name = _attributes$i.name, value = _attributes$i.value;
      serialized[name] = value;
    }
    return serialized;
  }
  var AmpImaVideo = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpImaVideo2, _AMP$BaseElement);
    var _super = _createSuper(AmpImaVideo2);
    function AmpImaVideo2(element) {
      var _this;
    ...
    _proto.buildCallback = function buildCallback() {
      ...
      assertHttpsUrl(this.element.getAttribute("data-tag"), "The data-tag attribute is required for <amp-video-ima> and must be https");
      var sourceElements = childElementsByTag(this.element, "SOURCE");
      var trackElements = childElementsByTag(this.element, "TRACK");
      var childElements = toArray(sourceElements).concat(toArray(trackElements));
      this.sourceChildren_ = childElements.map(function(element) {
        var tagName = element.tagName;
        var src = element.getAttribute("src");
        if (tagName == "SOURCE" && !_this2.preconnectSource_) {
          _this2.preconnectSource_ = src;
        } else if (tagName == "TRACK" && !_this2.preconnectTrack_) {
          _this2.preconnectTrack_ = src;
        }
        return [tagName, serializeAttributes(element)];
      });
      var scriptElement = childElementsByTag(this.element, "SCRIPT")[0];
      if (scriptElement && isJsonScriptTag(scriptElement)) {
        this.element.setAttribute("data-ima-settings", scriptElement.innerHTML);
      }
    };
    _proto.preconnectCallback = function preconnectCallback() {
      var element = this.element;
      var preconnect = Services.preconnectFor(this.win);
      preconnect.preload(this.getAmpDoc(), "https://imasdk.googleapis.com/js/sdkloader/ima3.js", "script");
    ...
    _proto.layoutCallback = function layoutCallback() {
      ...
      var consentPromise = consentPolicyId ? getConsentPolicyState(element, consentPolicyId) : Promise.resolve(null);
      element.setAttribute("data-source-children", JSON.stringify(this.sourceChildren_));
      return consentPromise.then(function(initialConsentState) {
        var iframe = getIframe(win, element, TYPE, {
          initialConsentState: initialConsentState
        }, {
          allowFullscreen: true
        });
        iframe.title = _this3.element.title || "IMA video";
        ...
        _this3.unlistenMessage_ = listen(_this3.win, "message", function(e) {
          return _this3.handlePlayerMessage_(e);
        });
        element.appendChild(iframe);
        installVideoManagerForDoc(element);
        Services.videoManagerForDoc(element).register(_this3);
        observeContentSize(_this3.element, _this3.onResized_);
      ...
      if (this.playerReadyPromise_) {
        this.playerReadyPromise_.then(function() {
          if (_this4.iframe_ && _this4.iframe_.contentWindow) {
            _this4.iframe_.contentWindow.postMessage(JSON.stringify(dict({
              "event": "command",
              "func": command,
              "args": opt_args || ""
            })), "*");
    ...
    _proto.handlePlayerMessage_ = function handlePlayerMessage_(event) {
      ...
      if (videoEvent == ImaPlayerData.IMA_PLAYER_DATA) {
        this.playerData_ = eventData["data"];
    ...
    _proto.createPlaceholderCallback = function createPlaceholderCallback() {
      var poster = this.element.dataset.poster;
      if (!poster) {
        return null;
      }
      var img = new Image();
    ...
    _proto.seekTo = function seekTo(unusedTimeSeconds) {
      this.user().error(TAG4, "`seekTo` not supported.");
    };
    return AmpImaVideo2;
  }(AMP.BaseElement);
  AMP.registerElement(TAG4, AmpImaVideo);
})();
