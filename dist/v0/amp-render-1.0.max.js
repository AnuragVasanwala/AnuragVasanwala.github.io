(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-render",ev:"1.0",l:true,f:(function(AMP,_){
(function() {
  ...
  // src/preact/component/renderer.js
  function useRenderer(renderer, data) {
    var _useState = useState(null), value = _useState[0], setValue = _useState[1];
    useLayoutEffect(function() {
  ...
  // extensions/amp-render/1.0/component.js
  var _excluded2 = ["src", "getJson", "render", "ariaLiveValue", "onLoading", "onLoad", "onRefresh", "onError"];
  function _extends7() {
  ...
  var DEFAULT_RENDER = function DEFAULT_RENDER2(data) {
    return JSON.stringify(data);
  };
  var DEFAULT_GET_JSON = function DEFAULT_GET_JSON2(url) {
    return fetch(url).then(function(res) {
      return res.json();
    });
  };
  function RenderWithRef(_ref, ref) {
    var _ref$src = _ref.src, src = _ref$src === void 0 ? "" : _ref$src, _ref$getJson = _ref.getJson, getJson = _ref$getJson === void 0 ? DEFAULT_GET_JSON : _ref$getJson, _ref$render = _ref.render, render2 = _ref$render === void 0 ? DEFAULT_RENDER : _ref$render, _ref$ariaLiveValue = _ref.ariaLiveValue, ariaLiveValue = _ref$ariaLiveValue === void 0 ? "polite" : _ref$ariaLiveValue, onLoading = _ref.onLoading, onLoad = _ref.onLoad, onRefresh = _ref.onRefresh, onError = _ref.onError, rest = _objectWithoutPropertiesLoose4(_ref, _excluded2);
    useResourcesNotify();
    var _useState = useState({}), data = _useState[0], setData = _useState[1];
    var onLoadRef = useValueRef(onLoad);
    var onErrorRef = useValueRef(onError);
    useEffect(function() {
      if (!src) {
        return;
      }
      var cancelled = false;
      onLoading == null ? void 0 : onLoading();
      getJson(src).then(function(data2) {
        if (!cancelled) {
          setData(data2);
        }
      }).catch(function(e3) {
        onErrorRef.current == null ? void 0 : onErrorRef.current(e3);
      });
      return function() {
        cancelled = true;
      };
    ...
    var refresh = useCallback(function() {
      ...
      getJson(src, true).then(function(data2) {
        setData(data2);
    ...
    var rendered = useRenderer(render2, data);
    var isHtml = rendered && typeof rendered == "object" && "__html" in rendered;
    var refFn = useCallback(function(node) {
      if (!(node != null && node.firstElementChild) || !rendered) {
        return;
      }
      onLoadRef.current == null ? void 0 : onLoadRef.current();
    }, [rendered, onLoadRef]);
    return createElement(Wrapper, _extends7({
      ref: refFn
    }, rest, {
  ...
  var Render = forwardRef(RenderWithRef);
  Render.displayName = "Render";

  // extensions/amp-render/1.0/base-element.js
  ...
  BaseElement2["Component"] = Render;
  BaseElement2["props"] = {
    "src": {
      attr: "src"
    }
  };
  BaseElement2["usesTemplate"] = true;
  BaseElement2["lightDomTag"] = "div";
  ...
  // extensions/amp-render/1.0/amp-render.js
  ...
  var isAmpStateSrc = function isAmpStateSrc2(src) {
  ...
  function getAmpStateJson(element, src) {
    return Services.bindForDocOrNull(element).then(function(bind) {
      ...
      return bind.getStateAsync(ampStatePath).catch(function(err) {
        ...
        user().error(TAG, "'amp-state' element with id '" + stateKey + "' was not found.");
        throw err;
      });
    }).then(function(json) {
      userAssert2(json !== void 0, "[amp-render] No data was found at provided uri: " + src);
      return json;
    });
  }
  ...
  function getTemplateNonEmptyNodeCount(doc, template) {
    ...
    if (template.tagName === "SCRIPT") {
      ...
      childNodes2 = div.childNodes;
    } else if (template.tagName == "TEMPLATE") {
      childNodes2 = template.content.childNodes;
    }
    return toArray(childNodes2).reduce(function(count, node) {
      return count + Number(node.nodeType === Node.TEXT_NODE ? node.textContent.trim().length > 0 : node.nodeType !== Node.COMMENT_NODE);
    }, 0);
  }
  var AmpRender = /* @__PURE__ */ function(_BaseElement) {
    _inherits4(AmpRender2, _BaseElement);
    var _super = _createSuper4(AmpRender2);
    function AmpRender2(element) {
      var _this;
      _this = _super.call(this, element);
      _this.templates_ = null;
      _this.template_ = null;
      _this.initialSrc_ = null;
      _this.src_ = null;
      return _this;
    }
    var _proto = AmpRender2.prototype;
    _proto.getPolicy_ = function getPolicy_() {
      var src = this.element.getAttribute("src");
      var policy = UrlReplacementPolicy_Enum.OPT_IN;
    ...
    _proto.buildOptionsObject_ = function buildOptionsObject_(shouldRefresh) {
      var _this$element$getAttr;
      if (shouldRefresh === void 0) {
        shouldRefresh = false;
      }
      return {
        xssiPrefix: this.element.getAttribute("xssi-prefix"),
        expr: (_this$element$getAttr = this.element.getAttribute("key")) != null ? _this$element$getAttr : ".",
        refresh: shouldRefresh,
        urlReplacement: this.getPolicy_()
      };
    };
    _proto.getFetchJsonFn = function getFetchJsonFn() {
      var _this2 = this;
      ...
      if (isAmpStateSrc(src)) {
        return function(src2) {
          return getAmpStateJson(element, src2);
        };
      }
      if (isAmpScriptUri(src)) {
        return function(src2) {
          return Services.scriptForDocOrNull(element).then(function(ampScriptService) {
            userAssert2(ampScriptService, "AMP-SCRIPT is not installed");
            return ampScriptService.fetch(src2);
          });
        };
      }
      return function(unusedSrc, shouldRefresh) {
        if (shouldRefresh === void 0) {
          shouldRefresh = false;
        }
        return batchFetchJsonFor(element.getAmpDoc(), element, _this2.buildOptionsObject_(shouldRefresh));
      };
    };
    _proto.isLayoutSupported = function isLayoutSupported(layout) {
      if (layout === Layout_Enum.CONTAINER) {
        userAssert2(this.getPlaceholder(), 'placeholder required with layout="container"');
      }
      return true;
    };
    _proto.handleResizeToContentsAction_ = function handleResizeToContentsAction_() {
      var _this3 = this;
      var currentHeight, targetHeight;
      this.measureMutateElement(function() {
        currentHeight = _this3.element.offsetHeight;
        targetHeight = _this3.element.scrollHeight;
        if (targetHeight < currentHeight) {
          var container = _this3.element.querySelector("div[i-amphtml-rendered]");
          targetHeight = container.scrollHeight;
          if (container.firstElementChild) {
            var marginTop = computedStyle(_this3.getAmpDoc().win, container.firstElementChild).getPropertyValue("margin-top");
            targetHeight += parseInt(marginTop, 10);
          }
          if (container.lastElementChild) {
            var marginBottom = computedStyle(_this3.getAmpDoc().win, container.lastElementChild).getPropertyValue("margin-bottom");
            targetHeight += parseInt(marginBottom, 10);
          }
        }
      }, function() {
        _this3.forceChangeHeight(targetHeight);
      });
    };
    _proto.init = function init() {
      var _this4 = this;
      ...
      var hasAriaLive = this.element.hasAttribute("aria-live");
      if (!hasAriaLive) {
        this.element.setAttribute("aria-live", "polite");
      }
      this.registerApiAction("refresh", function(api) {
        var src = _this4.element.getAttribute("src");
        if (!src || isAmpStateSrc(src) || isAmpScriptUri(src)) {
          return;
        }
        api.refresh();
      });
      this.registerAction("resizeToContents", function() {
        _this4.handleResizeToContentsAction_();
      });
      return dict({
        "ariaLiveValue": hasAriaLive ? this.element.getAttribute("aria-live") : "polite",
    ...
    _proto.handleOnLoad = function handleOnLoad() {
      ...
      var componentHeight, contentHeight;
      this.measureMutateElement(function() {
        componentHeight = computedStyle(_this5.getAmpDoc().win, _this5.element).getPropertyValue("height");
        contentHeight = _this5.element.querySelector("[i-amphtml-rendered]").scrollHeight;
      }, function() {
        setStyles(_this5.element, {
          "overflow": "hidden",
          "height": componentHeight
        });
      }).then(function() {
        return _this5.attemptChangeHeight(contentHeight).then(function() {
    ...
    _proto.renderTemplateAsString_ = function renderTemplateAsString_(data) {
      return this.templates_.renderTemplateAsString(dev().assertElement(this.template_), data).then(function(html) {
        return dict({
          "__html": html
        });
      });
    ...
    _proto.checkPropsPostMutations = function checkPropsPostMutations() {
      var _this6 = this;
      var templates = this.templates_ || (this.templates_ = Services.templatesForDoc(this.element));
      var template = templates.maybeFindTemplate(this.element);
      ...
      templates.whenReady(template).then(function() {
        if (template != _this6.template_) {
          return;
        }
        _this6.mutateProps(dict({
          "render": function render2(data) {
            var bindingValue = _this6.element.getAttribute("binding");
            if (bindingValue === Binding.NEVER || bindingValue === Binding.NO) {
              return _this6.renderTemplateAsString_(data);
            }
            return Services.bindForDocOrNull(_this6.element).then(function(bind) {
              if (!bind) {
                return _this6.renderTemplateAsString_(data);
              }
              var nonEmptyNodeCount = getTemplateNonEmptyNodeCount(_this6.element.ownerDocument, template);
              return templates.renderTemplate(dev().assertElement(template), data).then(function(element) {
                return bind.rescan([element], [], {
                  "fast": true,
                  "update": getUpdateValue(bindingValue, bind.signals().get("FIRST_MUTATE") === null)
                }).then(function() {
                  return dict({
                    "__html": nonEmptyNodeCount === 1 ? element.outerHTML : element.innerHTML
                  });
                });
              });
            });
    ...
    _proto.isReady = function isReady(props) {
...
//# sourceMappingURL=amp-render-1.0.max.js.map
