(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-nested-menu",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  var TAG = "amp-nested-menu";
  var Side = {
    LEFT: "left",
    RIGHT: "right"
  };
  var ANIMATION_TIMEOUT = 350;
  var AmpNestedMenu = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpNestedMenu2, _AMP$BaseElement);
    var _super = _createSuper(AmpNestedMenu2);
    AmpNestedMenu2.prerenderAllowed = function prerenderAllowed() {
      return true;
    };
    function AmpNestedMenu2(element) {
      var _this;
      _this = _super.call(this, element);
      _this.document_ = _this.win.document;
      _this.action_ = null;
      _this.side_ = null;
      _this.currentSubmenu_ = element;
      _this.clickHandler_ = _this.handleClick_.bind(_assertThisInitialized(_this));
      _this.keydownHandler_ = _this.handleKeyDown_.bind(_assertThisInitialized(_this));
      return _this;
    }
    var _proto = AmpNestedMenu2.prototype;
    ...
    _proto.registerSubmenuElements_ = function registerSubmenuElements_() {
      var _this2 = this;
      var submenuBtns = toArray(this.element.querySelectorAll("[amp-nested-submenu-open],[amp-nested-submenu-close]"));
      submenuBtns.forEach(function(submenuBtn) {
        ...
        userAssert2(_this2.action_.hasAction(submenuBtn, "tap") == false, "submenu open/close buttons should not have tap actions registered.");
      });
    };
    ...
    _proto.handleClick_ = function handleClick_(e) {
      ...
      var submenuBtn = closestAncestorElementBySelector(dev().assertElement(e.target), "[amp-nested-submenu-open],[amp-nested-submenu-close]");
      if (submenuBtn && this.shouldHandleClick_(target, submenuBtn)) {
        var isOpenBtn = submenuBtn.hasAttribute("amp-nested-submenu-open");
        if (isOpenBtn) {
    ...
    _proto.shouldHandleClick_ = function shouldHandleClick_(target, submenuBtn) {
      var hasAnchor = !!closest(target, function(e) {
        return e.tagName == "A";
      }, submenuBtn);
      var hasTapAction = this.action_.hasAction(target, "tap", submenuBtn);
      return !hasAnchor && !hasTapAction;
    };
    _proto.handleSubmenuOpenClick_ = function handleSubmenuOpenClick_(openElement) {
      var submenuParent = dev().assertElement(openElement.parentElement);
      var submenu = scopedQuerySelector(submenuParent, ">[amp-nested-submenu]");
      userAssert2(submenu, TAG + " requires each submenu open button to have a submenu as its sibling");
      this.open_(submenu);
    };
    _proto.open_ = function open_(submenu) {
      var _this3 = this;
      if (submenu.hasAttribute("open")) {
        return;
      }
      var parentMenu = this.getParentMenu_(submenu);
      if (parentMenu) {
        ...
        Services.timerFor(this.win).delay(function() {
          var submenuParent = dev().assertElement(submenu.parentElement);
          var submenuOpen = dev().assertElement(scopedQuerySelector(submenuParent, ">[amp-nested-submenu-open]"));
          submenuOpen.setAttribute("aria-expanded", "true");
          var submenuCloseCandidates = toArray(submenu.querySelectorAll("[amp-nested-submenu-close]"));
          var submenuClose = submenuCloseCandidates.filter(function(candidate) {
            return _this3.getParentMenu_(candidate) == submenu;
          })[0];
          userAssert2(submenuClose, TAG + " requires each submenu to contain at least one submenu close button");
          tryFocus(submenuClose);
        }, ANIMATION_TIMEOUT);
      }
    };
    _proto.handleSubmenuCloseClick_ = function handleSubmenuCloseClick_(closeElement) {
      var submenu = closestAncestorElementBySelector(closeElement, "[amp-nested-submenu]");
      userAssert2(submenu, TAG + ": submenu close buttons are only allowed inside submenus");
      this.close_(submenu);
    };
    _proto.close_ = function close_(submenu) {
      if (!submenu.hasAttribute("open")) {
        return;
      }
      var parentMenu = this.getParentMenu_(submenu);
    ...
    _proto.handleMenuNavigation_ = function handleMenuNavigation_(e) {
      var back = e.key == Keys_Enum.LEFT_ARROW;
    ...
    _proto.handleMenuItemNavigation_ = function handleMenuItemNavigation_(e) {
      var target = dev().assertElement(e.target);
      var parentMenu = this.getParentMenu_(target);
      var item = closest(target, function(e2) {
        return e2.tagName == "LI";
      }, parentMenu);
      if (!item) {
        return;
      }
      ...
      var focusElement = nextItem.querySelector("button,a[href],[tabindex]");
      if (focusElement) {
        e.preventDefault();
        tryFocus(focusElement);
      }
    };
    _proto.getParentMenu_ = function getParentMenu_(element) {
      return closestAncestorElementBySelector(dev().assertElement(element.parentElement), "amp-nested-menu,[amp-nested-submenu]");
    };
    return AmpNestedMenu2;
  }(AMP.BaseElement);
  AMP.registerElement(TAG, AmpNestedMenu, CSS2);
})();
/*! https://mths.be/cssescape v1.5.1 by @mathias | MIT license */
})});
//# sourceMappingURL=amp-nested-menu-0.1.max.js.map
