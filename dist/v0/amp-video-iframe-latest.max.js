(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-video-iframe",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
    ...
    return Services.consentPolicyServiceForDocOrNull(element).then(function(policy) {
      ...
      return Promise.all(Object.keys(gettersOrNull).map(function(key) {
        return gettersOrNull[key].call(policy, opt_policyId || "default").then(function(value) {
          var _key;
          return _key = {}, _key[key] = value, _key;
        });
      })).then(function(objs) {
  ...
  var SANDBOX = [SandboxOptions_Enum.ALLOW_SCRIPTS, SandboxOptions_Enum.ALLOW_SAME_ORIGIN, SandboxOptions_Enum.ALLOW_POPUPS, SandboxOptions_Enum.ALLOW_POPUPS_TO_ESCAPE_SANDBOX, SandboxOptions_Enum.ALLOW_TOP_NAVIGATION_BY_USER_ACTIVATION];
  var getAnalyticsEventTypePrefixRegex = once(function() {
    return new RegExp("^" + ANALYTICS_EVENT_TYPE_PREFIX);
  });
  var addDataParamsToUrl = function addDataParamsToUrl2(url, element) {
    return addParamsToUrl(url, getDataParamsFromAttributes(element));
  };
  ...
  var AmpVideoIframe = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpVideoIframe2, _AMP$BaseElement);
    var _super = _createSuper(AmpVideoIframe2);
    function AmpVideoIframe2(element) {
      var _this;
      _this = _super.call(this, element);
      _this.iframe_ = null;
      _this.unlistenFrame_ = null;
      _this.readyDeferred_ = null;
    ...
    _proto.layoutCallback = function layoutCallback() {
      var _this2 = this;
      this.user().assert(!looksLikeTrackingIframe(this.element), "<amp-video-iframe> does not allow tracking iframes. Please use amp-analytics instead.");
      var name = JSON.stringify(this.getMetadata_());
      this.iframe_ = disableScrollingOnIframe(createFrameFor(this, this.getSrc_(), name, SANDBOX));
    ...
    _proto.getMetadata_ = function getMetadata_() {
      var _Services$documentInf = Services.documentInfoForDoc(this.element), canonicalUrl = _Services$documentInf.canonicalUrl, sourceUrl = _Services$documentInf.sourceUrl;
      var rootNode = this.getAmpDoc().getRootNode();
      var documentElement = rootNode.documentElement, title = rootNode.title;
      return dict({
        "sourceUrl": sourceUrl,
        "canonicalUrl": canonicalUrl,
        "title": title || null,
        "lang": (documentElement == null ? void 0 : documentElement.lang) || null,
        "jsonLd": getJsonLd(rootNode)
      });
    };
    _proto.onReady_ = function onReady_() {
    ...
    _proto.removeIframe_ = function removeIframe_() {
      this.canPlay_ = false;
      if (this.iframe_) {
    ...
    _proto.getSrc_ = function getSrc_() {
      var element = this.element;
      var urlService = Services.urlForDoc(element);
      var src = element.getAttribute("src");
      if (urlService.getSourceOrigin(src) === urlService.getWinOrigin(this.win)) {
        user().warn(TAG2, "Origins of document inside amp-video-iframe and the host are the same, which allows for same-origin behavior. However in AMP cache, origins won't match. Please ensure you do not rely on any same-origin privileges.", element);
      }
      return maybeAddAmpFragment(addDataParamsToUrl(src, element));
    };
    _proto.createReadyPromise_ = function createReadyPromise_() {
      this.readyDeferred_ = new Deferred();
      return this.readyDeferred_.promise;
    };
    _proto.originMatches_ = function originMatches_(event) {
      return originMatches(event, this.iframe_, /.*/);
    };
    _proto.onMessage_ = function onMessage_(event) {
      var _this3 = this;
      if (!this.iframe_) {
        return;
      }
      ...
      if (methodReceived) {
        ...
        if (methodReceived == "getIntersection") {
          return measureIntersection(this.element).then(function(intersection) {
            _this3.postIntersection_(messageId, intersection);
          });
        }
        ...
        userAssert2(false, "Unknown method `%s`.", methodReceived);
        return;
      }
      var eventReceived = data["event"];
      var isCanPlayEvent = eventReceived == "canplay";
      this.canPlay_ = this.canPlay_ || isCanPlayEvent;
      var _devAssert = devAssert2(this.readyDeferred_), reject = _devAssert.reject, resolve = _devAssert.resolve;
      if (isCanPlayEvent) {
        return resolve();
      }
      ...
      if (eventReceived == "analytics") {
        var spec = devAssert2(data["analytics"]);
    ...
    _proto.dispatchCustomAnalyticsEvent_ = function dispatchCustomAnalyticsEvent_(eventType, vars) {
      ...
      userAssert2(getAnalyticsEventTypePrefixRegex().test(eventType), "Invalid analytics `eventType`. Value must start with `%s`.", ANALYTICS_EVENT_TYPE_PREFIX);
      dispatchCustomEvent(this.element, VideoEvents_Enum.CUSTOM_TICK, dict({
        "eventType": eventType,
        "vars": vars
      }));
    };
    _proto.postIntersection_ = function postIntersection_(messageId, intersection) {
      var intersectionRatio = intersection.intersectionRatio, time = intersection.time;
      var postedRatio = intersectionRatio < MIN_VISIBILITY_RATIO_FOR_AUTOPLAY ? 0 : intersectionRatio;
    ...
    _proto.postConsentData_ = function postConsentData_(messageId) {
      var _this4 = this;
      getConsentDataToForward(this.element, this.getConsentPolicy()).then(function(consentData) {
        _this4.postMessage_(dict({
          "id": messageId,
          "args": consentData
        }));
      });
    };
    _proto.method_ = function method_(method) {
      var _this5 = this;
      var _ref = this.readyDeferred_ || {}, promise = _ref.promise;
      if (!promise) {
        return;
      }
      promise.then(function() {
        _this5.postMessage_(dict({
          "event": "method",
          "method": method
        }));
      });
    };
    _proto.postMessage_ = function postMessage_(message) {
  ...
  AMP.registerElement(TAG2, AmpVideoIframe);
})();
