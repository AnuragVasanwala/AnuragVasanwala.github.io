(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-youtube",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  var AmpYoutube = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpYoutube2, _AMP$BaseElement);
    var _super = _createSuper(AmpYoutube2);
    function AmpYoutube2(element) {
      var _this;
    ...
    _proto.preconnectCallback = function preconnectCallback(opt_onLayout) {
      ...
      preconnect.url(ampdoc2, this.getVideoIframeSrc_());
      preconnect.url(ampdoc2, "https://s.ytimg.com", opt_onLayout);
      preconnect.url(ampdoc2, "https://i.ytimg.com", opt_onLayout);
    };
    ...
    _proto.getEmbedUrl_ = function getEmbedUrl_() {
      this.assertDatasourceExists_();
      var urlSuffix = this.getCredentials_() === "omit" ? "-nocookie" : "";
      var baseUrl = "https://www.youtube" + urlSuffix + ".com/embed/";
      var descriptor = this.videoid_ ? encodeURIComponent(this.videoid_ || "") + "?" : "live_stream?channel=" + encodeURIComponent(this.liveChannelid_ || "") + "&";
    ...
    _proto.getVideoIframeSrc_ = function getVideoIframeSrc_() {
      ...
      if ("loop" in params) {
        this.user().warn("AMP-YOUTUBE", "Use loop attribute instead of the deprecated data-param-loop");
      }
      this.isLoop_ = element.hasAttribute("loop") || "loop" in params && params["loop"] == "1";
    ...
    _proto.layoutCallback = function layoutCallback() {
      var _this2 = this;
      var iframe = createFrameFor(this, this.getVideoIframeSrc_());
      iframe.title = this.element.title || "YouTube video";
      addUnsafeAllowAutoplay(iframe);
      this.iframe_ = iframe;
      Services.videoManagerForDoc(this.element).register(this);
      this.unlistenMessage_ = listen(this.win, "message", this.handleYoutubeMessage_.bind(this));
      if (this.isLoop_ && !this.isPlaylist_) {
        this.unlistenLooping_ = listen(this.element, VideoEvents_Enum.ENDED, function(unusedEvent) {
          return _this2.play(false);
        });
      }
      var loaded = this.loadPromise(this.iframe_).then(function() {
        return Services.timerFor(_this2.win).promise(300);
      }).then(function() {
        _this2.listenToFrame_();
        dispatchCustomEvent(_this2.element, VideoEvents_Enum.LOAD);
      });
    ...
    _proto.getLiveChannelId_ = function getLiveChannelId_() {
    ...
    _proto.getCredentials_ = function getCredentials_() {
      return this.element.getAttribute("credentials") || "include";
    };
    _proto.assertDatasourceExists_ = function assertDatasourceExists_() {
      var datasourceExists = !(this.videoid_ && this.liveChannelid_) && (this.videoid_ || this.liveChannelid_);
      userAssert2(datasourceExists, "Exactly one of data-videoid or data-live-channelid should be present for <amp-youtube> %s", this.element);
    };
    _proto.sendCommand_ = function sendCommand_(command, opt_args) {
      ...
      this.playerReadyPromise_.then(function() {
        if (_this3.iframe_ && _this3.iframe_.contentWindow) {
          ...
          _this3.iframe_.contentWindow.postMessage(message, "*");
        }
      });
    };
    _proto.handleYoutubeMessage_ = function handleYoutubeMessage_(event) {
      ...
      if (eventType == "infoDelivery" && playerState != null) {
        ...
        redispatch(element, playerState.toString(), (_redispatch = {}, _redispatch[PlayerStates.PLAYING] = VideoEvents_Enum.PLAYING, _redispatch[PlayerStates.PAUSED] = VideoEvents_Enum.PAUSE, _redispatch[PlayerStates.ENDED] = [VideoEvents_Enum.ENDED, VideoEvents_Enum.PAUSE], _redispatch));
        return;
      }
      ...
      if (eventType == "infoDelivery" && info["currentTime"] !== void 0) {
        this.info_.currentTime = info["currentTime"];
        return;
      }
    };
    _proto.listenToFrame_ = function listenToFrame_() {
      if (!this.iframe_) {
        return;
      }
    ...
    _proto.createPlaceholderCallback = function createPlaceholderCallback() {
      ...
      var el = this.element;
      var imgPlaceholder = htmlFor(el)(_templateObject4 || (_templateObject4 = _taggedTemplateLiteralLoose3(["<img placeholder referrerpolicy=origin>"])));
  ...
  AMP.registerElement(TAG2, AmpYoutube);
})();
