(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-auto-lightbox",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  function getServiceForDoc(elementOrAmpDoc, id) {
    var ampdoc3 = getAmpdoc(elementOrAmpDoc);
    var holder = getAmpdocServiceHolder(ampdoc3);
  ...
  function getAmpdocServiceHolder(nodeOrDoc) {
    var ampdoc3 = getAmpdoc(nodeOrDoc);
    return ampdoc3.isSingleDoc() ? ampdoc3.win : ampdoc3;
  }
  ...
  function getElementServiceIfAvailableForDoc(element, id, extension, opt_element) {
    ...
    var ampdoc3 = getAmpdoc(element);
    return ampdoc3.whenExtensionsKnown().then(function() {
      var version2 = ampdoc3.getExtensionVersion(extension);
      if (!version2) {
        return null;
      }
      var extensions = getService(ampdoc3.win, "extensions");
  ...
  var Services = {
    ...
    analyticsForDoc: function analyticsForDoc(element, loadAnalytics) {
      ...
      if (loadAnalytics) {
        var ampdoc3 = getAmpdoc(element);
        extensionsForInternal(ampdoc3.win).installExtensionForDoc(ampdoc3, "amp-analytics");
      }
    ...
    storageForTopLevelDoc: function storageForTopLevelDoc(elementOrAmpDoc) {
      ...
      var ampdoc3 = topAmpdoc && topAmpdoc.win == thisAmpdoc.win ? topAmpdoc : thisAmpdoc;
      return getServicePromiseForDoc(ampdoc3, "storage");
    },
  ...
  var Criteria = /* @__PURE__ */ function() {
    ...
    Criteria2.meetsAll = function meetsAll(element, ampdoc3, renderWidth, renderHeight) {
      return Criteria2.meetsSizingCriteria(element, ampdoc3, renderWidth, renderHeight) && Criteria2.meetsTreeShapeCriteria(element, ampdoc3);
    };
    Criteria2.meetsTreeShapeCriteria = function meetsTreeShapeCriteria(element, ampdoc3) {
      ...
      var actions = Services.actionServiceForDoc(ampdoc3 || element);
      return !actions.hasResolvableAction(element, "tap");
    };
    Criteria2.meetsSizingCriteria = function meetsSizingCriteria(element, ampdoc3, renderWidth, renderHeight) {
      var _getMaxNaturalDimensi = getMaxNaturalDimensions(dev().assertElement(element.querySelector("img") || element)), naturalHeight = _getMaxNaturalDimensi.naturalHeight, naturalWidth = _getMaxNaturalDimensi.naturalWidth;
      var viewport = Services.viewportForDoc(ampdoc3);
  ...
  function whenLoaded(element) {
    ...
    return whenUpgradedToCustomElement(element).then(function(element2) {
      return element2.signals().whenSignal(CommonSignals_Enum.LOAD_END);
    });
  }
  var Scanner = /* @__PURE__ */ function() {
    function Scanner2() {
    }
    Scanner2.getCandidates = function getCandidates(root) {
  ...
  var DocMetaAnnotations = /* @__PURE__ */ function() {
    ...
    DocMetaAnnotations2.getOgType = function getOgType(ampdoc3) {
      var tag = ampdoc3.getRootNode().querySelector(META_OG_TYPE);
      if (tag) {
        return tag.getAttribute("content");
      }
    };
    DocMetaAnnotations2.hasValidOgType = function hasValidOgType(ampdoc3) {
      return DocMetaAnnotations2.getOgType(ampdoc3) == ENABLED_OG_TYPE_ARTICLE;
    };
    DocMetaAnnotations2.getAllLdJsonTypes = function getAllLdJsonTypes(ampdoc3) {
      return toArray(ampdoc3.getRootNode().querySelectorAll(SCRIPT_LD_JSON)).map(function(el) {
        var textContent = el.textContent;
        return (tryParseJson(textContent) || {})["@type"];
      }).filter(Boolean);
    };
    DocMetaAnnotations2.hasValidLdJsonType = function hasValidLdJsonType(ampdoc3) {
      return DocMetaAnnotations2.getAllLdJsonTypes(ampdoc3).some(function(type) {
        return ENABLED_LD_JSON_TYPES[type];
      });
    };
    return DocMetaAnnotations2;
  }();
  function usesLightboxExplicitly(ampdoc3) {
    ...
    var exists = function exists2(selector) {
      return !!ampdoc3.getRootNode().querySelector(selector);
    };
    return exists(requiredExtensionSelector) && exists(lightboxedElementsSelector);
  }
  function isEnabledForDoc(ampdoc3) {
    if (usesLightboxExplicitly(ampdoc3)) {
      return false;
    }
    return DocMetaAnnotations.hasValidOgType(ampdoc3) || DocMetaAnnotations.hasValidLdJsonType(ampdoc3);
  }
  var uid = 0;
  function generateLightboxUid() {
    return "i-amphtml-auto-lightbox-" + uid++;
  }
  function apply(ampdoc3, element) {
    var mutator = Services.mutatorForDoc(ampdoc3);
    ...
    return mutatePromise.then(function() {
      Services.extensionsFor(ampdoc3.win).installExtensionForDoc(ampdoc3, REQUIRED_EXTENSION);
      dispatchCustomEvent(element, AutoLightboxEvents_Enum.NEWLY_SET);
      return element;
    });
  }
  function runCandidates(ampdoc3, candidates) {
    return candidates.map(function(candidate) {
      return whenLoaded(candidate).then(function() {
        return measureIntersectionNoRoot(candidate).then(function(_ref) {
          ...
          if (candidate.tagName !== "IMG" && !candidate.signals().get(CommonSignals_Enum.LOAD_END)) {
            return;
          }
          var height = boundingClientRect.height, width = boundingClientRect.width;
          if (!Criteria.meetsAll(candidate, ampdoc3, width, height)) {
            return;
          }
          dev().info(TAG, "apply", candidate);
          return apply(ampdoc3, candidate);
        });
      }, NOOP);
    });
  }
  function scan(ampdoc3, opt_root) {
    if (!isEnabledForDoc(ampdoc3)) {
      dev().info(TAG, "disabled");
      return;
    }
    var root = opt_root || ampdoc3.win.document;
    return runCandidates(ampdoc3, Scanner.getCandidates(root));
  }
  var _AMP = AMP;
  var ampdoc2 = _AMP.ampdoc;
  ampdoc2.whenReady().then(function() {
    ampdoc2.getRootNode().addEventListener(AmpEvents_Enum.DOM_UPDATE, function(e) {
      var target = e.target;
      scan(ampdoc2, dev().assertElement(target));
    });
    scan(ampdoc2);
  });
})();
