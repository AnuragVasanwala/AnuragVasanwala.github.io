(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-ad-network-fake-impl",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  function adoptServiceForEmbedDoc(ampdoc2, id) {
    var service = getServiceInternal(getAmpdocServiceHolder(devAssert(ampdoc2.getParent())), id);
    registerServiceInternal(getAmpdocServiceHolder(ampdoc2), ampdoc2, id, function() {
      return service;
    }, false, true);
  }
  function adoptServiceFactoryForEmbedDoc(ampdoc2, id) {
    var parentHolder = getAmpdocServiceHolder(devAssert(ampdoc2.getParent()));
    ...
    registerServiceInternal(getAmpdocServiceHolder(ampdoc2), ampdoc2, id, devAssert(service.ctor));
  }
  ...
  var AmpAdMetadataTransformer = /* @__PURE__ */ function() {
    function AmpAdMetadataTransformer2() {
      this.metadata = dict({});
      this.styles_ = [];
      this.extensions_ = [];
    ...
    var _proto = AmpAdMetadataTransformer2.prototype;
    _proto.generateMetadata = function generateMetadata(doc) {
      ...
      var creative = doc.documentElement.outerHTML;
    ...
    _proto.generateJsonMetadata_ = function generateJsonMetadata_(doc) {
      ...
      for (var i = 0; i < json.length; i++) {
        ...
        if (script.hasAttribute("amp-ad-metadata")) {
          var parsed = parseJson(script.textContent);
    ...
    _proto.generateRuntimeOffsets_ = function generateRuntimeOffsets_(creative) {
      ...
      var end = 0;
      if (this.firstRuntimeElement_ != null) {
        var firstRuntimeElementString = this.firstRuntimeElement_.outerHTML;
        var lastRuntimeElementString = this.lastRuntimeElement_.outerHTML;
    ...
    _proto.generateJsonOffsets_ = function generateJsonOffsets_(creative) {
      if (this.jsonMetadata_.length > 0) {
        ...
        for (var i = 0; i < this.jsonMetadata_.length; i++) {
          ...
          var nameElementString = this.ampAnalytics_.innerHTML;
    ...
    _proto.generateAmpAnalyticsOffsets_ = function generateAmpAnalyticsOffsets_(doc, creative) {
      ...
      if (ampAnalytics.length > 0) {
        ...
        for (var i = 0; i < ampAnalytics.length; i++) {
          ...
          var nameElementString = element.innerHTML;
          var jsonStart = creative.indexOf(nameElementString);
          var jsonEnd = jsonStart + nameElementString.length;
    ...
    return AmpAdMetadataTransformer2;
  }();

  // extensions/amp-ad-network-fake-impl/0.1/external-reorder-head-transformer.js
  var ExternalReorderHeadTransformer = /* @__PURE__ */ function() {
    function ExternalReorderHeadTransformer2() {
    ...
    var _proto = ExternalReorderHeadTransformer2.prototype;
    ...
    _proto.registerScript_ = function registerScript_(element) {
      ...
      if (isAsync && (src.startsWith(urls.cdn) && (endsWith(src, "/v0.js") || endsWith(src, "/amp4ads-v0.js")) || endsWith(src, "/dist/amp.js") || endsWith(src, "/dist/amp-inabox.js") || endsWith(src, "/dist/v0.js") || endsWith(src, "/dist/amp4ads-v0.js"))) {
        this.headComponents_.scriptAmpEngine = element;
        return;
      }
      if (isAsync && src.startsWith(urls.cdn + "/v0/amp-viewer-integration-gmail-") && endsWith(src, ".js")) {
        this.headComponents_.scriptGmailAmpViewer = element;
        return;
      }
      if (isAsync && (src.startsWith(urls.cdn + "/v0/amp-viewer-integration-") || src.startsWith(urls.cdn + "/viewer/google/v") && endsWith(src, ".js"))) {
    ...
    _proto.registerLink_ = function registerLink_(element) {
      ...
      if (rel == "stylesheet") {
        if (element.getAttribute("href").startsWith(urls.cdn) && endsWith(element.getAttribute("href"), "/v0.css")) {
    ...
    return ExternalReorderHeadTransformer2;
  }();
  ...
  var Resource = /* @__PURE__ */ function() {
    ...
    _proto.startLayout = function startLayout() {
      ...
      devAssert(this.state_ != ResourceState_Enum.NOT_BUILT, "Not ready to start layout: %s (%s)", this.debugid, this.state_);
  ...
  function actionTrustToString(actionTrust) {
    switch (actionTrust) {
      ...
      case ActionTrust_Enum.HIGH:
        return "high";
      default:
        devAssert2(actionTrust === ActionTrust_Enum.DEFAULT);
        return "default";
    }
  }
  ...
  var TAG15 = "AMP-AD-NETWORK-FAKE-IMPL";
  var EXPERIMENT_BRANCH_ATTR = "data-experiment-id";
  var AmpAdNetworkFakeImpl = /* @__PURE__ */ function(_AmpA4A) {
    _inherits5(AmpAdNetworkFakeImpl2, _AmpA4A);
    var _super = _createSuper5(AmpAdNetworkFakeImpl2);
    function AmpAdNetworkFakeImpl2(element) {
      var _this;
      _this = _super.call(this, element);
      _this.reorderHeadTransformer_ = new ExternalReorderHeadTransformer();
      _this.metadataTransformer_ = new AmpAdMetadataTransformer();
      return _this;
    }
    var _proto = AmpAdNetworkFakeImpl2.prototype;
    _proto.buildCallback = function buildCallback() {
      var _this2 = this;
      userAssert(this.element.hasAttribute("src") || this.element.hasAttribute("srcdoc"), 'Attribute src or srcdoc required for <amp-ad type="fake">: %s', this.element);
      if (this.element.hasAttribute(EXPERIMENT_BRANCH_ATTR)) {
        this.element.getAttribute(EXPERIMENT_BRANCH_ATTR).split(",").forEach(function(experiment) {
          var expParts = experiment.split(":");
          forceExperimentBranch(_this2.win, expParts[0], expParts[1]);
        });
      }
    ...
    _proto.isValidElement = function isValidElement() {
      ...
      if (!id || !id.startsWith("i-amphtml-demo-")) {
        user().warn(TAG15, "Only works with id starts with i-amphtml-demo-");
        return false;
      }
      return true;
    };
    ...
    _proto.sendXhrRequest = function sendXhrRequest(adUrl) {
      ...
      return _AmpA4A.prototype.sendXhrRequest.call(this, adUrl).then(function(response) {
        ...
        var _response = response, headers = _response.headers, status = _response.status;
        if (_this3.element.getAttribute("a4a-conversion") == "true") {
          return response.text().then(function(responseText) {
            if (includes(responseText, "?__amp_source_origin=")) {
              responseText = responseText.split("?__amp_source_origin=")[0];
            }
            return new Response(_this3.transformCreative_(responseText), {
              status: status,
              headers: headers
            });
          });
