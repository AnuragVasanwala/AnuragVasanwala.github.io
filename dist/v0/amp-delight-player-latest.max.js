(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-delight-player",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  // build/amp-delight-player-0.1.css.js
  var CSS2 = "amp-delight-player [placeholder]{transition:opacity 0.5s ease-out}amp-delight-player [placeholder].i-amphtml-delight-player-faded{opacity:0;pointer-events:none}\n/*# sourceURL=/extensions/amp-delight-player/0.1/amp-delight-player.css*/";
  ...
  var AmpDelightPlayer = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpDelightPlayer2, _AMP$BaseElement);
    var _super = _createSuper(AmpDelightPlayer2);
    function AmpDelightPlayer2(element) {
      var _this;
      _this = _super.call(this, element);
      _this.isInViewport_ = false;
      _this.baseURL_ = "https://players.delight-vr.com";
      _this.contentID_ = "";
      _this.totalDuration_ = 1;
      ...
      _this.unlistenScreenOrientationChange_ = null;
      _this.unlistenOrientationChange_ = null;
      _this.unlistenDeviceOrientation_ = null;
      _this.unlistenDeviceMotion_ = null;
      _this.placeholderEl_ = null;
      _this.pauseHelper_ = new PauseHelper(_this.element);
      _this.unobserveIntersections_ = null;
      return _this;
    }
    var _proto = AmpDelightPlayer2.prototype;
    ...
    _proto.buildCallback = function buildCallback() {
      this.contentID_ = userAssert2(this.element.getAttribute("data-content-id"), "The data-content-id attribute is required");
      var deferred = new Deferred();
    ...
    _proto.layoutCallback = function layoutCallback() {
      ...
      this.unobserveIntersections_ = observeIntersections(this.element, function(_ref) {
        ...
        return _this2.isInViewport_ = isIntersecting;
      });
      var src = this.baseURL_ + "/player/" + this.contentID_ + "?amp=1";
      var iframe = createFrameFor(this, src);
      iframe.setAttribute("allow", "vr");
      this.unlistenMessage_ = listen(this.win, "message", function(event) {
        _this2.handleDelightMessage_(event);
      });
      this.iframe_ = iframe;
    ...
    _proto.createPlaceholderCallback = function createPlaceholderCallback() {
      ...
      var placeholder = html2(_templateObject4 || (_templateObject4 = _taggedTemplateLiteralLoose3(['\n      <img placeholder referrerpolicy="origin" loading="lazy" />\n    '])));
      applyFillContent(placeholder);
      var src = this.baseURL_ + "/poster/" + this.contentID_;
    ...
    _proto.firstLayoutCompleted = function firstLayoutCompleted() {
      ...
      return promise.then(function() {
        return _AMP$BaseElement.prototype.firstLayoutCompleted.call(_this3);
      });
    };
    ...
    _proto.handleDelightMessage_ = function handleDelightMessage_(event) {
      var _redispatch;
      if (!originMatches(event, this.iframe_, /.*/)) {
        return;
      }
      var data = objOrParseJson(getData(event));
      if (!data || !data["type"]) {
        return;
      }
      ...
      var redispatched = redispatch(element, data["type"], (_redispatch = {}, _redispatch[DelightEvent.PLAYING] = VideoEvents_Enum.PLAYING, _redispatch[DelightEvent.PAUSED] = VideoEvents_Enum.PAUSE, _redispatch[DelightEvent.ENDED] = VideoEvents_Enum.ENDED, _redispatch[DelightEvent.MUTED] = VideoEvents_Enum.MUTED, _redispatch[DelightEvent.UNMUTED] = VideoEvents_Enum.UNMUTED, _redispatch[DelightEvent.AD_START] = VideoEvents_Enum.AD_START, _redispatch[DelightEvent.AD_END] = VideoEvents_Enum.AD_END, _redispatch));
      if (redispatched) {
        return;
      }
      switch (data["type"]) {
        ...
        case DelightEvent.DURATION: {
          ...
          this.totalDuration_ = _payload.duration;
          break;
        }
        ...
        case DelightEvent.CUSTOM_TICK: {
          var _payload2 = data["payload"];
          this.dispatchCustomAnalyticsEvent_(_payload2.type, _payload2);
          break;
        }
      }
    };
    _proto.dispatchCustomAnalyticsEvent_ = function dispatchCustomAnalyticsEvent_(eventType, vars) {
      dispatchCustomEvent(this.element, VideoEvents_Enum.CUSTOM_TICK, dict({
        "eventType": ANALYTICS_EVENT_TYPE_PREFIX + eventType,
        "vars": vars
      }));
    };
    _proto.sendCommand_ = function sendCommand_(type, payload) {
      if (payload === void 0) {
        payload = {};
      }
    ...
    _proto.setFullHeight_ = function setFullHeight_() {
      setStyle(this.iframe_, "position", "fixed");
    };
    _proto.setInlineHeight_ = function setInlineHeight_() {
      setStyle(this.iframe_, "position", "absolute");
    };
    _proto.registerEventHandlers_ = function registerEventHandlers_() {
      var _this4 = this;
      var dispatchScreenOrientationChangeEvents = function dispatchScreenOrientationChangeEvents2() {
        var orientation = window.screen.orientation || window.screen.mozOrientation || window.screen.msOrientation;
        _this4.sendCommand_(DelightEvent.SCREEN_CHANGE, {
          orientation: {
      ...
      var dispatchOrientationChangeEvents = function dispatchOrientationChangeEvents2() {
        var _window = window, orientation = _window.orientation;
        _this4.sendCommand_(DelightEvent.WINDOW_ORIENTATIONCHANGE, {
          orientation: orientation
        });
      };
      var dispatchDeviceOrientationEvents = function dispatchDeviceOrientationEvents2(event) {
        _this4.sendCommand_(DelightEvent.WINDOW_DEVICEORIENTATION, {
          alpha: event.alpha,
          beta: event.beta,
          gamma: event.gamma,
      ...
      var dispatchDeviceMotionEvents = function dispatchDeviceMotionEvents2(event) {
        var payload = {
        ...
        _this4.sendCommand_(DelightEvent.WINDOW_DEVICEMOTION, payload);
      };
      if (window.screen) {
        var screen = window.screen.orientation || window.screen.mozOrientation || window.screen.msOrientation;
        if (screen && screen.addEventListener) {
          this.unlistenScreenOrientationChange_ = listen(screen, "change", dispatchScreenOrientationChangeEvents);
        } else {
          this.unlistenOrientationChange_ = listen(this.win, "orientationchange", dispatchOrientationChangeEvents);
        }
      } else {
        this.unlistenOrientationChange_ = listen(this.win, "orientationchange", dispatchOrientationChangeEvents);
      }
      this.unlistenDeviceOrientation_ = listen(this.win, "deviceorientation", dispatchDeviceOrientationEvents);
      this.unlistenDeviceMotion_ = listen(this.win, "devicemotion", dispatchDeviceMotionEvents);
    };
    _proto.unregisterEventHandlers_ = function unregisterEventHandlers_() {
    ...
    _proto.sendConsentData_ = function sendConsentData_() {
      var _this5 = this;
      var consentPolicyId = _AMP$BaseElement.prototype.getConsentPolicy.call(this) || "default";
      var consentStringPromise = getConsentPolicyInfo(this.element, consentPolicyId);
      var metadataPromise = getConsentMetadata(this.element, consentPolicyId);
      var consentPolicyStatePromise = getConsentPolicyState(this.element, consentPolicyId);
      var consentPolicySharedDataPromise = getConsentPolicySharedData(this.element, consentPolicyId);
      Promise.all([metadataPromise, consentStringPromise, consentPolicyStatePromise, consentPolicySharedDataPromise]).then(function(consents) {
        _this5.sendCommand_(DelightEvent.CONSENT_DATA, {
    ...
    _proto.seekTo = function seekTo(time) {
      this.sendCommand_(DelightEvent.SEEK, {
        time: time
      });
    };
    return AmpDelightPlayer2;
  }(AMP.BaseElement);
  AMP.registerElement(TAG2, AmpDelightPlayer, CSS2);
})();
/*! https://mths.be/cssescape v1.5.1 by @mathias | MIT license */
})});
//# sourceMappingURL=amp-delight-player-0.1.max.js.map
