(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-fx-flying-carpet",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  var AmpFlyingCarpet = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpFlyingCarpet2, _AMP$BaseElement);
    var _super = _createSuper(AmpFlyingCarpet2);
    function AmpFlyingCarpet2(element) {
      var _this;
      _this = _super.call(this, element);
      _this.children_ = [];
      _this.totalChildren_ = 0;
      _this.container_ = null;
      _this.initialPositionChecked_ = false;
      return _this;
    }
    var _proto = AmpFlyingCarpet2.prototype;
    ...
    _proto.buildCallback = function buildCallback() {
      ...
      var childNodes2 = realChildNodes(this.element);
      this.totalChildren_ = this.visibileChildren_(childNodes2).length;
      var owners = Services.ownersForDoc(this.element);
      this.children_.forEach(function(child) {
        return owners.setOwner(child, _this2.element);
      });
      ...
      childNodes2.forEach(function(child) {
    ...
    _proto.assertPosition_ = function assertPosition_() {
      ...
      userAssert2(layoutBox.top >= minTop, "<amp-fx-flying-carpet> elements must be positioned after the 75% of first viewport: %s Current position: %s. Min: %s", this.element, layoutBox.top, minTop);
      userAssert2(layoutBox.top <= maxTop, "<amp-fx-flying-carpet> elements must be positioned before the last viewport: %s Current position: %s. Max: %s", this.element, layoutBox.top, maxTop);
    };
    _proto.layoutCallback = function layoutCallback() {
      ...
      Services.ownersForDoc(this.element).scheduleLayout(this.element, this.children_);
      this.observeNewChildren_();
      this.forceClipDraw_();
      return resolvedPromise();
    };
    _proto.forceClipDraw_ = function forceClipDraw_() {
      var _this3 = this;
      var inob = new this.win.IntersectionObserver(function(entries) {
        var last = entries[entries.length - 1];
        _this3.container_.classList.toggle("i-amphtml-fx-flying-carpet-container-fix", last.isIntersecting);
      }, {
        threshold: 0.01
      });
      inob.observe(this.element);
    };
    _proto.observeNewChildren_ = function observeNewChildren_() {
      var _this4 = this;
      var observer = new MutationObserver(function(changes) {
        for (var i = 0; i < changes.length; i++) {
          var addedNodes = changes[i].addedNodes;
          if (!addedNodes) {
            continue;
          }
          for (var n = 0; n < addedNodes.length; n++) {
            ...
            node.signals().whenSignal(CommonSignals_Enum.BUILT).then(_this4.layoutBuiltChild_.bind(_this4, node));
          }
        }
      });
    ...
    _proto.layoutBuiltChild_ = function layoutBuiltChild_(node) {
      var child = dev().assertElement(node);
      if (child.getOwner() === this.element) {
        Services.ownersForDoc(this.element).scheduleLayout(this.element, child);
      }
    };
    _proto.collapsedCallback = function collapsedCallback(child) {
    ...
    _proto.visibileChildren_ = function visibileChildren_(nodes) {
    ...
    return AmpFlyingCarpet2;
  }(AMP.BaseElement);
  AMP.registerElement(TAG, AmpFlyingCarpet, CSS2);
})();
