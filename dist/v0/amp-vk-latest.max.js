(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-vk",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  var AmpVk = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpVk2, _AMP$BaseElement);
    var _super = _createSuper(AmpVk2);
    function AmpVk2(element) {
      var _this;
    ...
    _proto.getVkPostIFrameSrc_ = function getVkPostIFrameSrc_() {
      var _this2 = this;
      return Services.viewerForDoc(this.element).getReferrerUrl().then(function(ref) {
        var startWidth = _this2.element.offsetWidth;
    ...
    _proto.getVkPollIFrameSrc_ = function getVkPollIFrameSrc_() {
      var _this3 = this;
      return Services.viewerForDoc(this.element).getReferrerUrl().then(function(ref) {
    ...
    _proto.buildCallback = function buildCallback() {
      this.embedType_ = userAssert2(this.element.getAttribute("data-embedtype"), "The data-embedtype attribute is required for <amp-vk> %s", this.element);
      userAssert2(isEnumValue(EmbedType, this.embedType_), "Unknown data-embedtype: " + this.embedType_);
      if (this.embedType_ === EmbedType.POST) {
    ...
    _proto.postBuildCallback_ = function postBuildCallback_() {
      this.ownerId_ = userAssert2(this.element.getAttribute("data-owner-id"), "The data-owner-id attribute is required for <amp-vk> Post %s", this.element);
      this.postId_ = userAssert2(this.element.getAttribute("data-post-id"), "The data-post-id attribute is required for <amp-vk> Post %s", this.element);
      this.hash_ = userAssert2(this.element.getAttribute("data-hash"), "The data-hash attribute is required for <amp-vk> Post %s", this.element);
    };
    _proto.pollBuildCallback_ = function pollBuildCallback_() {
      this.apiId_ = userAssert2(this.element.getAttribute("data-api-id"), "The data-api-id attribute is required for <amp-vk> Poll %s", this.element);
      this.pollId_ = userAssert2(this.element.getAttribute("data-poll-id"), "The data-poll-id attribute is required for <amp-vk> Poll %s", this.element);
    };
    _proto.layoutCallback = function layoutCallback() {
      var _this4 = this;
      var iframe = this.element.ownerDocument.createElement("iframe");
      this.iframe_ = iframe;
      this.unlistenMessage_ = listen(this.win, "message", this.handleVkIframeMessage_.bind(this));
    ...
    _proto.handleVkIframeMessage_ = function handleVkIframeMessage_(e) {
      if (e.origin !== "https://vk.com" || e.source !== this.iframe_.contentWindow) {
        return;
      }
      var eventData = getData(e);
      if (eventData) {
        var regExp = /\[\"resize",\[(\d+)\]]/;
        var matches2 = regExp.exec(eventData);
        if (matches2 && matches2[1]) {
          var newHeight = parseInt(matches2[1], 10);
    ...
    _proto.isLayoutSupported = function isLayoutSupported(layout) {
      return layout === Layout_Enum.RESPONSIVE || layout === Layout_Enum.FLEX_ITEM || layout === Layout_Enum.FIXED;
    };
