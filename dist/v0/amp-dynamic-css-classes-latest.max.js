(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-dynamic-css-classes",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  function referrerDomain(ampdoc2) {
    var referrer = Services.viewerForDoc(ampdoc2).getUnconfirmedReferrerUrl();
    if (!referrer) {
      return "";
    }
    var _Services$urlForDoc$p = Services.urlForDoc(ampdoc2.getHeadNode()).parse(referrer), hostname = _Services$urlForDoc$p.hostname;
    return hostname;
  }
  ...
  function normalizedReferrers(ampdoc2) {
    var referrer = referrerDomain(ampdoc2);
    if (referrer === "t.co") {
      return referrers_("twitter.com");
    }
    if (!referrer && /Pinterest/.test(userAgent(ampdoc2.win))) {
      return referrers_("www.pinterest.com");
    }
    return referrers_(referrer);
  }
  function addDynamicCssClasses(ampdoc2, classes) {
    if (ampdoc2.isBodyAvailable()) {
      addCssClassesToBody(ampdoc2.getBody(), classes);
    } else {
  ...
  function addReferrerClasses(ampdoc2) {
    var referrers = normalizedReferrers(ampdoc2);
    var classes = referrers.map(function(referrer) {
      return "amp-referrer-" + referrer.replace(/\./g, "-");
    });
    Services.vsyncFor(ampdoc2.win).mutate(function() {
      addDynamicCssClasses(ampdoc2, classes);
    });
  }
  function addViewerClass(ampdoc2) {
    var viewer = Services.viewerForDoc(ampdoc2);
    if (viewer.isEmbedded()) {
      Services.vsyncFor(ampdoc2.win).mutate(function() {
        addDynamicCssClasses(ampdoc2, ["amp-viewer"]);
      });
    }
  }
  function addRuntimeClasses(ampdoc2) {
    addReferrerClasses(ampdoc2);
    addViewerClass(ampdoc2);
  }
  var AmpDynamicCssClasses = /* @__PURE__ */ function() {
    function AmpDynamicCssClasses2(ampdoc2) {
      addRuntimeClasses(ampdoc2);
    }
    var _proto = AmpDynamicCssClasses2.prototype;
    _proto.whenReady = function whenReady() {
      return resolvedPromise();
    };
    return AmpDynamicCssClasses2;
  }();
