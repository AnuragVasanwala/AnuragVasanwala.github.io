(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-install-serviceworker",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  var AmpInstallServiceWorker = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpInstallServiceWorker2, _AMP$BaseElement);
    var _super = _createSuper(AmpInstallServiceWorker2);
    function AmpInstallServiceWorker2(element) {
      var _this;
      _this = _super.call(this, element);
      _this.iframeSrc_ = null;
      _this.urlRewriter_ = null;
      _this.isSafari_ = Services.platformFor(_this.win).isSafari();
      return _this;
    }
    var _proto = AmpInstallServiceWorker2.prototype;
    _proto.buildCallback = function buildCallback() {
      ...
      if ((urlService.isProxyOrigin(src) || urlService.isProxyOrigin(win.location.href)) && !this.isSafari_) {
        var iframeSrc = this.element.getAttribute("data-iframe-src");
        if (iframeSrc) {
          urlService.assertHttpsUrl(iframeSrc, this.element);
          var _urlService$parse = urlService.parse(iframeSrc), origin = _urlService$parse.origin;
          var docInfo = Services.documentInfoForDoc(this.element);
          var sourceUrl = urlService.parse(docInfo.sourceUrl);
          var canonicalUrl = urlService.parse(docInfo.canonicalUrl);
          userAssert(origin == sourceUrl.origin || origin == canonicalUrl.origin, "data-iframe-src (%s) should be a URL on the same origin as the source (%s) or canonical URL (%s) of the AMP-document.", origin, sourceUrl.origin, canonicalUrl.origin);
        ...
        this.whenLoadedAndVisiblePromise_().then(function() {
          return install(_this2.win, src, _this2.element);
        });
      } else {
        this.user().error(TAG, "Did not install ServiceWorker because it does not match the current origin: " + src);
      }
      if ((urlService.isProxyOrigin(src) || urlService.isProxyOrigin(win.location.href)) && this.isSafari_) {
        this.user().error(TAG, "Did not install ServiceWorker because of safari double keyring caching as it will not have any effect");
      }
    };
    _proto.whenLoadedAndVisiblePromise_ = function whenLoadedAndVisiblePromise_() {
      return Promise.all([this.loadPromise(this.win), this.getAmpDoc().whenFirstVisible()]);
    };
    _proto.insertIframe_ = function insertIframe_() {
      var _this3 = this;
      return this.mutateElement(function() {
        toggle(_this3.element, false);
    ...
    _proto.maybeInstallUrlRewrite_ = function maybeInstallUrlRewrite_() {
      ...
      userAssert(urlMatch && shellUrl, 'Both, "%s" and "%s" must be specified for url-rewrite', "data-no-service-worker-fallback-url-match", "data-no-service-worker-fallback-shell-url");
      shellUrl = removeFragment(shellUrl);
      var urlMatchExpr;
      try {
        urlMatchExpr = new RegExp(urlMatch);
      } catch (e) {
        throw user().createError('Invalid "data-no-service-worker-fallback-url-match" expression', e);
      }
      userAssert(urlService.getSourceOrigin(winUrl) == urlService.parse(shellUrl).origin, 'Shell source origin "%s" must be the same as source origin "%s"', shellUrl, winUrl.href);
      this.urlRewriter_ = new UrlRewriter_(ampdoc2, urlMatchExpr, shellUrl, this.element);
      if (urlService.isSecure(shellUrl)) {
        this.waitToPreloadShell_(shellUrl);
      }
    };
    _proto.waitToPreloadShell_ = function waitToPreloadShell_(shellUrl) {
      var _this4 = this;
    ...
    _proto.preloadShell_ = function preloadShell_(shellUrl) {
      var win = this.win;
    ...
    return AmpInstallServiceWorker2;
  }(AMP.BaseElement);
  var UrlRewriter_ = /* @__PURE__ */ function() {
    function UrlRewriter_2(ampdoc2, urlMatchExpr, shellUrl, element) {
      ...
      listen(ampdoc2.getRootNode(), "click", this.handle_.bind(this));
    }
    var _proto2 = UrlRewriter_2.prototype;
    _proto2.handle_ = function handle_(event) {
      if (event.defaultPrevented) {
        return;
      }
      var target = closestAncestorElementBySelector(dev().assertElement(event.target), "A");
  ...
  function install(win, src, element) {
    ...
    return win.navigator.serviceWorker.register(src, options).then(function(registration) {
      if (getMode().development) {
        user().info(TAG, "ServiceWorker registration successful with scope: ", registration.scope);
      }
      var installingSw = registration.installing;
      if (installingSw) {
        installingSw.addEventListener("statechange", function(evt) {
  ...
  function sendAmpScriptToSwOnFirstVisit(win, registration) {
    if ("performance" in win) {
      var ampScriptsUsed = win.performance.getEntriesByType("resource").filter(function(item) {
        return item.initiatorType === "script" && item.name.startsWith(urls.cdn);
      }).map(function(script) {
        return script.name;
      });
  ...
  function prefetchOutgoingLinks(registration, win) {
    ...
    var links = [].map.call(document.querySelectorAll("a[data-rel=prefetch]"), function(link) {
      return link.href;
    });
  ...
  function supportsPrefetch(doc) {
    var fakeLink = doc.createElement("link");
