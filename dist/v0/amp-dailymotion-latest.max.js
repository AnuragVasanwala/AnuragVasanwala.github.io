(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-dailymotion",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  function getDailymotionIframeSrc(win, videoId, autoplay, endscreenEnable, info, mute, sharingEnable, start, uiHighlight, uiLogo, implicitParams) {
    if (autoplay === void 0) {
      autoplay = false;
    }
    if (endscreenEnable === void 0) {
    ...
    if (sharingEnable === void 0) {
      sharingEnable = true;
    }
    if (uiLogo === void 0) {
      uiLogo = true;
    }
    return addParamsToUrl("https://www.dailymotion.com/embed/video/" + encodeURIComponent(videoId) + "?api=1&html=1&app=amp", Object.assign(dict({
      "endscreen-enable": !endscreenEnable ? endscreenEnable : void 0,
      "info": !info ? info : void 0,
      "mute": mute || autoplay && isAutoplaySupported(win) ? 1 : void 0,
      "sharing-enable": !sharingEnable ? sharingEnable : void 0,
      "start": start,
      "ui-highlight": uiHighlight,
      "ui-logo": !uiLogo ? uiLogo : void 0
    }), implicitParams));
  }
  function makeDailymotionMessage(command, param) {
  ...
  var TAG2 = "amp-dailymotion";
  var AmpDailymotion = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpDailymotion2, _AMP$BaseElement);
    var _super = _createSuper(AmpDailymotion2);
    function AmpDailymotion2(element) {
      var _this;
    ...
    var _proto = AmpDailymotion2.prototype;
    ...
    _proto.buildCallback = function buildCallback() {
      this.videoid_ = userAssert(this.element.getAttribute("data-videoid"), "The data-videoid attribute is required for <amp-dailymotion> %s", this.element);
    ...
    _proto.handleEvents_ = function handleEvents_(event) {
      ...
      var data = parseQueryString(eventData);
      if (data === void 0) {
        return;
      }
      redispatch(this.element, data["event"], (_redispatch = {}, _redispatch[DailymotionEvents.API_READY] = VideoEvents_Enum.LOAD, _redispatch[DailymotionEvents.END] = [VideoEvents_Enum.ENDED, VideoEvents_Enum.PAUSE], _redispatch[DailymotionEvents.PAUSE] = VideoEvents_Enum.PAUSE, _redispatch[DailymotionEvents.PLAY] = VideoEvents_Enum.PLAYING, _redispatch));
      switch (data["event"]) {
    ...
    _proto.sendCommand_ = function sendCommand_(command, opt_arg) {
      ...
      this.playerReadyPromise_.then(function() {
        if (_this2.iframe_ && _this2.iframe_.contentWindow) {
          _this2.iframe_.contentWindow.postMessage(makeDailymotionMessage(command, opt_arg), endpoint);
        }
      });
    };
    _proto.getIframeSrc_ = function getIframeSrc_() {
      var _this$element$dataset = this.element.dataset, endscreenEnable = _this$element$dataset["endscreenEnable"], info = _this$element$dataset["info"], mute = _this$element$dataset["mute"], sharingEnable = _this$element$dataset["sharingEnable"], start = _this$element$dataset["start"], uiHighlight = _this$element$dataset["uiHighlight"], uiLogo = _this$element$dataset["uiLogo"];
      return getDailymotionIframeSrc(this.win, this.videoid_, this.element.hasAttribute("autoplay"), endscreenEnable !== "false", info !== "false", mute === "true", sharingEnable !== "false", start, uiHighlight, uiLogo !== "false", getDataParamsFromAttributes(this.element));
    };
    ...
    _proto.mute = function mute() {
      ...
      this.playerReadyPromise_.then(function() {
        dispatchCustomEvent(_this4.element, VideoEvents_Enum.MUTED);
        _this4.muted_ = true;
      });
    };
    _proto.unmute = function unmute() {
      ...
      this.playerReadyPromise_.then(function() {
        dispatchCustomEvent(_this5.element, VideoEvents_Enum.UNMUTED);
        _this5.muted_ = false;
      });
    };
    _proto.showControls = function showControls() {
    ...
    return AmpDailymotion2;
  }(AMP.BaseElement);
  AMP.registerElement(TAG2, AmpDailymotion);
})();
