(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-orientation-observer",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  // extensions/amp-orientation-observer/0.1/amp-orientation-observer.js
  function _extends() {
  ...
  var DELTA_CONST = 0.1;
  var DEFAULT_SMOOTHING_PTS = 4;
  var AmpOrientationObserver = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpOrientationObserver2, _AMP$BaseElement);
    var _super = _createSuper(AmpOrientationObserver2);
    function AmpOrientationObserver2(element) {
      var _this;
      _this = _super.call(this, element);
      _this.ampdoc_ = Services.ampdoc(_this.element);
      _this.action_ = null;
      _this.range_ = _extends({}, DEFAULT_RANGES);
      _this.computedValue_ = _extends({}, DEFAULT_REST_VALUES);
      _this.restValues_ = _extends({}, DEFAULT_REST_VALUES);
      _this.smoothingPoints_ = {
        beta: [],
        alpha: [],
        gamma: []
      };
      _this.smoothing_ = null;
      return _this;
    }
    var _proto = AmpOrientationObserver2.prototype;
    ...
    _proto.init_ = function init_() {
      var _this2 = this;
      userAssert(this.win.DeviceOrientationEvent, "The current browser doesn't support the `window.DeviceOrientationEvent`");
      AXES.forEach(function(axis) {
        _this2.range_[axis] = _this2.parseAttributes_(axis + "-range", _this2.range_[axis]);
      });
      this.smoothing_ = this.element.hasAttribute("smoothing") ? Number(this.element.getAttribute("smoothing")) || DEFAULT_SMOOTHING_PTS : null;
      this.win.addEventListener("deviceorientation", function(event) {
        _this2.deviceOrientationHandler_(event);
      }, true);
    };
    _proto.parseAttributes_ = function parseAttributes_(rangeName, originalRange) {
    ...
    _proto.deviceOrientationHandler_ = function deviceOrientationHandler_(event) {
      var _this3 = this;
      if (event instanceof DeviceOrientationEvent) {
        var screen = this.win.screen;
        var alpha = event.alpha;
        var beta = event.beta, gamma = event.gamma;
        var angle = "orientation" in screen ? screen.orientation.angle : screen.orientation;
        ...
        AXES.forEach(function(axis) {
          if (Math.abs(currentValue[axis] - _this3.computedValue_[axis]) > DELTA_CONST) {
            if (_this3.smoothing_) {
              _this3.computedValue_[axis] = _this3.smoothedValue_(axis, currentValue[axis]);
            } else {
              _this3.computedValue_[axis] = currentValue[axis];
            }
            _this3.triggerEvent_(axis, _this3.computedValue_[axis], _this3.range_[axis]);
          }
        });
      }
    };
    _proto.smoothedValue_ = function smoothedValue_(axis, value) {
      ...
      var avg = sum(this.smoothingPoints_[axis]) / this.smoothing_;
      if (this.smoothingPoints_[axis].length > this.smoothing_ && this.restValues_[axis] == DEFAULT_REST_VALUES[axis]) {
        this.restValues_[axis] = avg;
      }
      return avg - this.restValues_[axis];
    };
    _proto.triggerEvent_ = function triggerEvent_(eventName, eventValue, eventRange) {
      var percentValue = eventRange[0] < 0 ? eventValue.toFixed() - eventRange[0] : eventValue.toFixed();
      var event = createCustomEvent(this.win, TAG + "." + eventName, dict({
    ...
    return AmpOrientationObserver2;
  }(AMP.BaseElement);
