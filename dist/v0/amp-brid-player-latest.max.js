(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-brid-player",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  var TAG2 = "amp-brid-player";
  var AmpBridPlayer = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpBridPlayer2, _AMP$BaseElement);
    var _super = _createSuper(AmpBridPlayer2);
    function AmpBridPlayer2(element) {
      var _this;
    ...
    var _proto = AmpBridPlayer2.prototype;
    ...
    _proto.buildCallback = function buildCallback() {
      ...
      this.partnerID_ = userAssert2(element.getAttribute("data-partner"), "The data-partner attribute is required for <amp-brid-player> %s", element);
      this.playerID_ = userAssert2(element.getAttribute("data-player"), "The data-player attribute is required for <amp-brid-player> %s", element);
      this.feedID_ = userAssert2(element.getAttribute("data-video") || element.getAttribute("data-playlist") || element.getAttribute("data-carousel") || element.getAttribute("data-outstream"), "Either the data-video, data-playlist, data-carousel or data-outstream attributes must be specified for <amp-brid-player> %s", element);
      var deferred = new Deferred();
    ...
    _proto.layoutCallback = function layoutCallback() {
      ...
      this.iframe_ = iframe;
      this.unlistenMessage_ = listen(this.win, "message", this.handleBridMessage_.bind(this));
    ...
    _proto.createPlaceholderCallback = function createPlaceholderCallback() {
      ...
      var feedID = this.feedID_, partnerID = this.partnerID_;
      ...
      this.loadPromise(placeholder).catch(function() {
    ...
    _proto.sendCommand_ = function sendCommand_(command, opt_arg) {
      ...
      this.playerReadyPromise_.then(function() {
        if (_this3.iframe_ && _this3.iframe_.contentWindow) {
          var args = opt_arg === void 0 ? "" : "|" + opt_arg;
    ...
    _proto.getConsentData_ = function getConsentData_() {
      ...
      var consentStatePromise = getConsentPolicyState(this.element, consentPolicy);
      var consentStringPromise = getConsentPolicyInfo(this.element, consentPolicy);
      return Promise.all([consentStatePromise, consentStringPromise]).then(function(consents) {
        var consentData;
        switch (consents[0]) {
        ...
        _this4.sendCommand_("setAMPGDPR", JSON.stringify(consentData));
      });
    };
    ...
    return AmpBridPlayer2;
  }(AMP.BaseElement);
  AMP.registerElement(TAG2, AmpBridPlayer);
})();
