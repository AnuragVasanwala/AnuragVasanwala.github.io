(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-ad-network-doubleclick-impl",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  var RealTimeConfigManager = /* @__PURE__ */ function() {
    ...
    _proto2.buildErrorResponse_ = function buildErrorResponse_(error, callout, errorReportingUrl, opt_rtcTime) {
      dev().warn(TAG2, "RTC callout to " + callout + " caused " + error);
      ...
      if (isArray(sendRegardlessOfConsentState)) {
        for (var i = 0; i < sendRegardlessOfConsentState.length; i++) {
          if (this.consentState_ == CONSENT_POLICY_STATE[sendRegardlessOfConsentState[i]]) {
            ...
            dev().warn(TAG2, "Invalid RTC consent state given: " + ("" + sendRegardlessOfConsentState[i]));
          }
        }
        return false;
      }
      user().warn(TAG2, "Invalid value for sendRegardlessOfConsentState:" + ("" + sendRegardlessOfConsentState));
      return !!optIsGloballyValid;
    };
    ...
    _proto2.handleRtcForCustomUrls = function handleRtcForCustomUrls(customMacros, checkStillCurrent, element) {
      ...
      (this.rtcConfig_.urls || []).forEach(function(urlObj) {
        ...
        if (isObject(urlObj)) {
          ...
          dev().warn(TAG2, "Invalid url: " + urlObj);
        }
      ...
      Object.keys(this.rtcConfig_.vendors || []).forEach(function(vendor) {
        ...
        Object.keys(vendorMacros).forEach(function(macro) {
          if (!(vendorObject.macros && vendorObject.macros.includes(macro))) {
            user().error(TAG2, "Unknown macro: " + macro + " for vendor: " + vendor);
          } else {
    ...
    _proto2.inflateAndSendRtc_ = function inflateAndSendRtc_(url, macros, errorReportingUrl, checkStillCurrent, opt_vendor, opt_element) {
      ...
      var allowlist = _extends2({}, GLOBAL_MACRO_ALLOWLIST);
    ...
    _proto2.validateRtcConfig_ = function validateRtcConfig_(element) {
      ...
      if (!rtcConfig) {
        user().warn(TAG2, "Could not JSON parse rtc-config attribute");
        return false;
      }
      var timeout;
      try {
        ...
        Object.keys(rtcConfig).forEach(function(key) {
          switch (key) {
            ...
            case "timeoutMillis":
              ...
              if (isNaN(timeout)) {
                user().warn(TAG2, "Invalid RTC timeout is NaN, " + ("using default timeout " + defaultTimeoutMillis + "ms"));
                timeout = void 0;
              } else if (timeout > defaultTimeoutMillis || timeout < 0) {
                user().warn(TAG2, "Invalid RTC timeout: " + timeout + "ms, " + ("using default timeout " + defaultTimeoutMillis + "ms"));
                timeout = void 0;
              }
              break;
            default:
              user().warn(TAG2, "Unknown RTC Config key: " + key);
              break;
          }
        });
        ...
        var validateErrorReportingUrl = function validateErrorReportingUrl2(urlObj) {
          ...
          if (errorUrl && !Services.urlForDoc(_this7.ampDoc_).isSecure(errorUrl)) {
            dev().warn(TAG2, "Insecure RTC errorReportingUrl: " + errorUrl);
  ...
  function lineDelimitedStreamer(win, response, lineCallback) {
    ...
    function streamer(text, done) {
      ...
      while (match = regex2.exec(text)) {
        line += match[1];
        if (match[2]) {
          lineCallback(line, done && regex2.lastIndex === text.length);
          line = "";
        }
        if (regex2.lastIndex === text.length) {
          break;
        }
      }
    }
    ...
    reader.read().then(function chunk(result) {
      if (result.value) {
        streamer(decoder.decode(result.value, {
          "stream": true
        }), result.done);
      }
      if (!result.done) {
  ...
  function metaJsonCreativeGrouper(callback) {
    ...
    return function(line, done) {
      if (first) {
        var metadata = tryParseJson(first) || {};
  ...
  function unescapeLineDelimitedHtml_(html2) {
    return html2.replace(/\\(n|r|\\)/g, function(_, match) {
      return match == "n" ? "\n" : match == "r" ? "\r" : "\\";
    });
  }
  ...
  var VariableSource = /* @__PURE__ */ function() {
    ...
    _proto.getExpr = function getExpr(opt_bindings, opt_allowlist) {
      ...
      var all = _extends3({}, this.replacements_, opt_bindings);
  ...
  function groupAmpAdsByType(ampdoc2, type, groupFn) {
    var ampAdSelector = function ampAdSelector2(r) {
      return r.element.querySelector("amp-ad[type=" + type + "]");
    };
    return getMeasuredResources(ampdoc2, ampdoc2.win, function(r) {
      ...
      return elements2.reduce(function(result, element) {
  ...
  function truncAndTimeUrl(baseUrl, parameters, startTime) {
    return buildUrl(baseUrl, parameters, MAX_URL_LENGTH2 - 10, TRUNCATION_PARAM) + "&dtd=" + elapsedTimeWithCeiling(Date.now(), startTime);
  }
  ...
  function maybeAppendErrorParameter(adUrl, parameterValue) {
    ...
    devAssert2(modifiedAdUrl.length <= MAX_URL_LENGTH2);
  ...
  function installGlobalNavigationHandlerForDoc(ampdoc2) {
    registerServiceBuilderForDoc(ampdoc2, TAG3, Navigation, true);
  }
  var Navigation = /* @__PURE__ */ function() {
    ...
    _proto.navigateTo = function navigateTo(win, url, opt_requestedBy, options) {
      ...
      if (!urlService.isProtocolValid(url)) {
        user().error(TAG3, "Cannot navigate to invalid protocol: " + url);
        return;
      }
    ...
    _proto.removeViewerQueryBeforeNavigation_ = function removeViewerQueryBeforeNavigation_(win, fromLocation, target) {
      dev().info(TAG3, "Removing iframe query string before navigation:", fromLocation.search);
      ...
      var restoreQuery = function restoreQuery2() {
        ...
        if (currentHref == noQuery) {
          dev().info(TAG3, "Restored iframe URL with query string:", original);
          win.history.replaceState(null, "", original);
        } else {
          dev().error(TAG3, "Unexpected iframe URL change:", currentHref, noQuery);
        }
      };
      if (target === "_blank") {
    ...
    _proto.scrollToElement_ = function scrollToElement_(elem, hash) {
      ...
      if (elem) {
        ...
        dev().warn(TAG3, "failed to find element with id=" + hash + " or a[name=" + hash + "]");
      }
    };
  ...
  // src/utils/dom-ancestor-visitor.js
  var DomAncestorVisitor = /* @__PURE__ */ function() {
    function DomAncestorVisitor2(win) {
      if (win === void 0) {
        win = window;
      }
      this.visitors_ = [];
    ...
    var _proto = DomAncestorVisitor2.prototype;
    ...
    return DomAncestorVisitor2;
  }();
  var Visitor = /* @__PURE__ */ function() {
    function Visitor2(callback, maxAncestorsToVisit) {
    ...
    var _proto2 = Visitor2.prototype;
    _proto2.callback = function callback(element, style) {
      ...
      if (!--this.maxAncestorsToVisit_ || result != void 0) {
        this.complete = true;
      }
    };
    return Visitor2;
  ...
  function getFlexibleAdSlotData(win, element) {
    ...
    var setFws = function setFws2(val) {
      return fwSignal = val;
    };
    var setMsz = function setMsz2(val) {
      return slotWidth = val;
    };
    var setPsz = function setPsz2(val) {
      return parentWidth = val;
    };
    new DomAncestorVisitor(win).addVisitor(getElementWidthVisitor(setMsz), 1).addVisitor(getElementWidthVisitor(setPsz), 100).addVisitor(getFullWidthSignalVisitor(setFws), 100).addVisitor(function(el, style) {
      return parentStyle = style;
    }, 1).visitAncestorsStartingFrom(element);
    return {
      fwSignal: fwSignal,
      slotWidth: slotWidth,
  ...
  var TAG4 = "AMP-DOUBLECLICK-SAFEFRAME";
  function safeframeListener(event) {
    ...
    if (!safeframeHost) {
      dev().warn(TAG4, "Safeframe Host for sentinel: " + sentinel + " not found.");
      return;
    }
    if (!safeframeHost.equalsSafeframeContentWindow(event.source)) {
      dev().warn(TAG4, "Safeframe source did not match event.source.");
      return;
    }
  ...
  var SafeframeHostApi = /* @__PURE__ */ function() {
    function SafeframeHostApi2(baseInstance, isFluid, creativeSize) {
      ...
      this.initialCreativeSize_ = _extends4({}, creativeSize);
      this.delay_ = null;
    ...
    var _proto = SafeframeHostApi2.prototype;
    _proto.equalsSafeframeContentWindow = function equalsSafeframeContentWindow(otherWindow) {
    ...
    _proto.connectMessagingChannel = function connectMessagingChannel(channel) {
      devAssert2(this.baseInstance_.iframe);
    ...
    _proto.setupGeom_ = function setupGeom_() {
      devAssert2(this.iframe_.contentWindow, "Frame contentWindow unavailable.");
      var throttledUpdate = throttle(this.win_, this.updateGeometry_.bind(this), 1e3);
    ...
    _proto.sendMessage_ = function sendMessage_(payload, serviceName) {
      if (!this.iframe_ || !this.iframe_.contentWindow) {
        dev().expectedError(TAG4, "Frame contentWindow unavailable.");
        return;
      }
      var message = dict();
      ...
      this.iframe_.contentWindow.postMessage(JSON.stringify(message), "*");
    };
    ...
    _proto.handleExpandRequest_ = function handleExpandRequest_(payload) {
      ...
      if (isNaN(expandHeight) || isNaN(expandWidth) || payload["push"] && !this.expandByPush_ || !payload["push"] && !this.expandByOverlay_ && (expandWidth > this.creativeSize_.width || expandHeight > this.creativeSize_.height)) {
        dev().error(TAG4, "Invalid expand values.");
        this.sendResizeResponse(false, SERVICE.EXPAND_RESPONSE);
        return;
      }
    ...
    _proto.handleCollapseRequest_ = function handleCollapseRequest_() {
      if (this.isCollapsed_ || !this.isRegistered_) {
        this.sendResizeResponse(false, SERVICE.COLLAPSE_RESPONSE);
        return;
      }
      this.handleSizeChange(this.initialCreativeSize_.height, this.initialCreativeSize_.width, SERVICE.COLLAPSE_RESPONSE, true);
    };
    _proto.resizeSafeframe = function resizeSafeframe(height, width, messageType) {
      ...
      this.baseInstance_.measureMutateElement(function() {
        ...
        _this2.sendResizeResponse(true, messageType);
      }, this.iframe_);
    };
    ...
    _proto.handleResizeRequest_ = function handleResizeRequest_(payload) {
      ...
      if (isNaN(resizeWidth) || isNaN(resizeHeight)) {
        dev().error(TAG4, "Invalid resize values.");
        return;
      }
    ...
    _proto.resizeAmpAdAndSafeframe = function resizeAmpAdAndSafeframe(height, width, messageType, opt_isShrinking) {
      ...
      this.baseInstance_.attemptChangeSize(height, width).then(function() {
        ...
        dev().error(TAG4, "Resizing failed: " + err);
        _this5.sendResizeResponse(false, messageType);
      });
    };
    _proto.handleFluidMessage_ = function handleFluidMessage_(payload) {
      ...
      this.baseInstance_.attemptChangeHeight(newHeight).then(function() {
        ...
        user().warn(TAG4, err);
        var _this6$baseInstance_$ = _this6.baseInstance_.getSlotSize(), height = _this6$baseInstance_$.height, width = _this6$baseInstance_$.width;
        if (width && height) {
  ...
  var Expander = /* @__PURE__ */ function() {
        ...
        while (urlIndex < url.length && matchIndex <= matches2.length) {
          ...
          if (match && urlIndex === match.start) {
            ...
            if (_this.bindings_ && hasOwn(_this.bindings_, match.name)) {
              ...
              binding = _extends5({}, _this.variableSource_.get(match.name), {
                name: match.name,
                encode: encode
              });
            }
    ...
    _proto.evaluateBinding_ = function evaluateBinding_(bindingInfo, opt_args) {
      ...
      if (bindingInfo.prioritized != void 0) {
        ...
        user().error(TAG5, "ignoring async replacement key: ", bindingInfo.name);
        binding = "";
      } else {
    ...
    _proto.evaluateBindingSync_ = function evaluateBindingSync_(binding, name, opt_args) {
      try {
        ...
        if (value && typeof value.then == "function") {
          user().error(TAG5, "ignoring async macro resolution");
          result = "";
  ...
  var GlobalVariableSource = /* @__PURE__ */ function(_VariableSource) {
        ...
        return Services.cidForDoc(_this3.ampdoc).then(function(cid) {
          ...
          if (cid && cookieName == "_ga") {
            if (typeof cid === "string") {
              ...
              dev().error(TAG6, "non-string cid, what is it?", Object.keys(cid));
            }
          }
          clientIds[scope] = cid;
          return cid;
    ...
    _proto.getAccessValue_ = function getAccessValue_(getter, expr) {
      ...
      return Promise.all([Services.accessServiceForDocOrNull(element), Services.subscriptionsServiceForDocOrNull(element)]).then(function(services) {
        ...
        if (!service) {
          user().error(TAG6, "Access or subsciptions service is not installed to access: ", expr);
          return null;
        }
  ...
  var UrlReplacements = /* @__PURE__ */ function() {
    ...
    _proto2.ensureProtocolMatches_ = function ensureProtocolMatches_(url, replacement) {
      ...
      if (newProtocol != oldProtocol) {
        user().error(TAG6, "Illegal replacement of the protocol: ", url);
        return url;
      }
  ...
  var A4AVariableSource = /* @__PURE__ */ function(_VariableSource) {
    ...
    _proto.htmlAttributeBinding_ = function htmlAttributeBinding_(cssSelector, var_args) {
      ...
      if (attributeNames.length > HTML_ATTR_MAX_ATTRS) {
        user().error(TAG19, "At most " + HTML_ATTR_MAX_ATTRS + " may be requested.");
        return "[]";
      }
      ...
      try {
        ...
        user().error(TAG19, "Invalid selector: " + cssSelector);
        return "[]";
      }
      if (elements2.length > HTML_ATTR_MAX_ELEMENTS_TO_TRAVERSE) {
        user().error(TAG19, "CSS selector may match at most " + (HTML_ATTR_MAX_ELEMENTS_TO_TRAVERSE + " elements."));
        return "[]";
      }
      var result = [];
  ...
  var Resource = /* @__PURE__ */ function() {
    ...
    _proto.maybeReportErrorOnBuildFailure = function maybeReportErrorOnBuildFailure(reason) {
      if (!isBlockedByConsent(reason)) {
        dev().error(TAG7, "failed to build:", this.debugid, reason);
      }
    };
    ...
    _proto.startLayout = function startLayout() {
      ...
      if (this.layoutCount_ > 0 && !this.element.isRelayoutNeeded()) {
        dev().fine(TAG7, "layout canceled since it wasn't requested:", this.debugid, this.state_);
        this.state_ = ResourceState_Enum.LAYOUT_COMPLETE;
        return resolvedPromise();
      }
      dev().fine(TAG7, "start layout:", this.debugid, "count:", this.layoutCount_);
      this.layoutCount_++;
      this.state_ = ResourceState_Enum.LAYOUT_SCHEDULED;
    ...
    _proto.layoutComplete_ = function layoutComplete_(success, signal, opt_reason) {
      ...
      if (success) {
        dev().fine(TAG7, "layout complete:", this.debugid);
      } else {
        dev().fine(TAG7, "loading failed:", this.debugid, opt_reason);
    ...
    _proto.getScopedCid = function getScopedCid(apiKey, scope) {
      ...
      return this.cidPromise_[scope] = this.timer_.poll(200, function() {
        ...
        return _this.fetchCid_(dev().assertString(url), scope, token).then(function(response) {
          ...
          if (e && e.response) {
            e.response.json().then(function(res) {
              dev().error(TAG10, JSON.stringify(res));
            });
          } else {
            dev().error(TAG10, e);
          }
  ...
  var HistoryBindingNatural_ = /* @__PURE__ */ function() {
    ...
    _proto3.mergeStateUpdate_ = function mergeStateUpdate_(state, update) {
      var mergedData = _extends7({}, state && state.data || {}, update.data || {});
      return _extends7({}, state || {}, update, {
        data: mergedData
      });
    };
  ...
  var MutatorImpl = /* @__PURE__ */ function() {
    ...
    _proto.dirtyElement = function dirtyElement(element) {
      ...
      var isAmpElement2 = element.classList.contains("i-amphtml-element");
      if (isAmpElement2) {
        var r = Resource.forElement(element);
    ...
    _proto.getStore_ = function getStore_() {
      if (!this.storePromise_) {
        this.storePromise_ = this.binding_.loadBlob(this.origin_).then(function(blob) {
          ...
          dev().expectedError(TAG12, "Failed to load store: ", reason);
          return {};
        }).then(function(obj) {
    ...
    _proto.listenToBroadcasts_ = function listenToBroadcasts_() {
      ...
      this.viewer_.onBroadcast(function(message) {
        if (message["type"] == "amp-storage-reset" && message["origin"] == _this2.origin_) {
          dev().fine(TAG12, "Received reset message");
          _this2.storePromise_ = null;
        }
      });
    };
    _proto.broadcastReset_ = function broadcastReset_() {
      dev().fine(TAG12, "Broadcasted reset message");
  ...
  var ViewerStorageBinding = /* @__PURE__ */ function() {
    ...
    _proto5.saveBlob = function saveBlob(origin, blob) {
      return this.viewer_.sendMessageAwaitResponse("saveStore", dict({
        ...
        throw dev().createExpectedError(TAG12, "Failed to save store: ", reason);
      });
    };
  ...
  function installTimerInEmbedWindow(embedWin) {
    registerServiceBuilderInEmbedWin(embedWin, TAG13, Timer);
  }
  ...
  function installUrlForDoc(ampdoc2) {
    registerServiceBuilderForDoc(ampdoc2, SERVICE2, Url, true);
  }
  ...
  function onDocumentFormSubmit_(e) {
    ...
    if (method == "GET") {
      ...
      if (action) {
        var TAG19 = "form";
        user().error(TAG19, "action attribute is invalid for method=POST: %s", form);
      }
      if (!actionXhr) {
        e.preventDefault();
  ...
  function applySandbox(iframe) {
    ...
    for (var i = 0; i < requiredFlags.length; i++) {
      ...
      if (!iframe.sandbox.supports(flag)) {
        dev().info(TAG14, "Iframe doesn't support %s", flag);
        return;
      }
    }
  ...
  var TAG16 = "amp-ad-network-doubleclick-impl";
  ...
  function combineInventoryUnits(impls) {
    ...
    impls.forEach(function(instance) {
      var iu = devAssert2(instance.element.getAttribute("data-slot"));
      var componentNames = iu.split("/").map(function(componentName) {
  ...
  function getSizes(impls) {
    return {
      "prev_iu_szs": impls.map(function(impl) {
        return devAssert2(impl.parameterSize);
      }).join()
    };
  }
  ...
  function getTargetingAndExclusions(impls) {
    ...
    var _loop = function _loop2(i2) {
      ...
      if (commonKVs) {
        ...
        commonKVs = _extends8({}, impl.jsonTargeting["targeting"]);
      }
    };
    for (var i = 0; i < impls.length; i++) {
      var _ret = _loop(i);
    ...
    var hasScp = false;
    var scps = [];
    var hasTargeting = function hasTargeting2(impl) {
  ...
  function serializeTargeting(targeting, categoryExclusions, commonTargeting) {
    var serialized = targeting ? Object.keys(targeting).filter(function(key) {
      return !commonTargeting || commonTargeting[key] === void 0;
    }).map(function(key) {
  ...
  function sraBlockCallbackHandler(creative, headersObj, done, sraRequestAdUrlResolvers, sraUrl, isNoSigning) {
    var headerNames = Object.keys(headersObj);
    if (headerNames.length == 1 && isObject(headersObj[headerNames[0]])) {
      headersObj = headersObj[headerNames[0]];
    ...
    devAssert2(sraRequestAdUrlResolvers.shift())(fetchResponse);
    if (done && sraRequestAdUrlResolvers.length) {
      dev().warn(TAG16, "Premature end of SRA response", sraRequestAdUrlResolvers.length, sraUrl);
    }
  }
  ...
  var TAG17 = "AMP-AD";
  function getPublisherSpecifiedRefreshInterval(element, unusedWin) {
  ...
  function checkAndSanitizeRefreshInterval(refreshInterval) {
    ...
    if (isNaN(refreshIntervalNum) || refreshIntervalNum < MIN_REFRESH_INTERVAL) {
      user().warn(TAG17, "invalid refresh interval, must be a number no less than " + (MIN_REFRESH_INTERVAL + ": " + refreshInterval));
      return null;
    }
    return refreshIntervalNum * 1e3;
  }
  ...
  var RefreshManager = /* @__PURE__ */ function() {
    function RefreshManager2(a4a, config, refreshInterval) {
    ...
    var _proto = RefreshManager2.prototype;
    ...
    _proto.convertAndSanitizeConfiguration_ = function convertAndSanitizeConfiguration_(config) {
      devAssert2(config["visiblePercentageMin"] >= 0 && config["visiblePercentageMin"] <= 100, "visiblePercentageMin for refresh must be in the range [0, 100]");
      config["continuousTimeMin"] *= 1e3;
  ...
  var TAG18 = "amp-ad-network-doubleclick-impl";
  ...
  var AmpAdNetworkDoubleclickImpl = /* @__PURE__ */ function(_AmpA4A) {
    _inherits5(AmpAdNetworkDoubleclickImpl2, _AmpA4A);
    var _super = _createSuper5(AmpAdNetworkDoubleclickImpl2);
    function AmpAdNetworkDoubleclickImpl2(element) {
      var _this;
      _this = _super.call(this, element);
      ...
      if ("forceSafeframe" in _this.element.dataset) {
        if (!/^(1|(true))$/i.test(_this.element.dataset["forceSafeframe"])) {
          user().warn(TAG18, "Ignoring invalid data-force-safeframe attribute: " + _this.element.dataset["forceSafeframe"]);
        } else {
    ...
    var _proto = AmpAdNetworkDoubleclickImpl2.prototype;
    ...
    _proto.setPageLevelExperiments = function setPageLevelExperiments(urlExperimentId) {
      ...
      var experimentInfoList = [{
        ...
        isTrafficEligible: function isTrafficEligible() {
          return !forcedExperimentId && !_this3.win.document.querySelector("meta[name=amp-ad-enable-refresh], amp-ad[type=doubleclick][data-enable-refresh], meta[name=amp-ad-doubleclick-sra]");
        },
    ...
    _proto.randomlySelectUnsetExperiments_ = function randomlySelectUnsetExperiments_(experimentInfoList) {
    ...
    _proto.maybeDeprecationWarn_ = function maybeDeprecationWarn_() {
      var warnDeprecation = function warnDeprecation2(feature) {
        return user().warn(TAG18, feature + " is no longer supported for DoubleClick.Please refer to https://github.com/ampproject/amphtml/issues/11834 for more information");
      };
    ...
    _proto.buildCallback = function buildCallback() {
      ...
      if (pubEnabledSra && delayFetchEnabled) {
        user().warn(TAG18, "SRA is not compatible with lazy fetching, disabling SRA");
      }
      ...
      this.identityTokenPromise_ = this.getAmpDoc().whenFirstVisible().then(function() {
        return getIdentityToken(_this4.win, _this4.getAmpDoc(), _AmpA4A.prototype.getConsentPolicy.call(_this4));
      });
    ...
    _proto.shouldPreferentialRenderWithoutCrypto = function shouldPreferentialRenderWithoutCrypto() {
      devAssert2(!isCdnProxy(this.win));
      return true;
    };
    _proto.getPageParameters = function getPageParameters(consentTuple, instances) {
      instances = instances || [this];
      var tokens = getPageviewStateTokensForAdRequest(instances);
      var additionalConsent = consentTuple.additionalConsent, consentString = consentTuple.consentString, consentStringType = consentTuple.consentStringType, gdprApplies = consentTuple.gdprApplies;
      return {
        "ptt": 13,
    ...
    _proto.getBlockParameters_ = function getBlockParameters_() {
      devAssert2(this.initialSize_);
      devAssert2(this.jsonTargeting);
      ...
      var _this$flexibleAdSlotD = this.flexibleAdSlotData_, fwSignal = _this$flexibleAdSlotD.fwSignal, parentWidth = _this$flexibleAdSlotD.parentWidth, slotWidth = _this$flexibleAdSlotD.slotWidth;
      if (this.uiHandler.isStickyAd()) {
        msz = "0x-1";
        psz = "0x-1";
      } else {
    ...
    _proto.populateAdUrlState = function populateAdUrlState(consentTuple) {
    ...
    _proto.getAdUrl = function getAdUrl(opt_consentTuple, opt_rtcResponsesPromise, opt_serveNpaSignal) {
      var _this5 = this;
      if (this.useSra) {
      ...
      if (consentTuple.consentState == CONSENT_POLICY_STATE.UNKNOWN && this.element.getAttribute("data-npa-on-unknown-consent") != "true") {
        user().info(TAG18, "Ad request suppressed due to unknown consent");
        this.getAdUrlDeferred.resolve("");
        return Promise.resolve("");
      }
      if (this.iframe && !this.isRefreshing) {
        dev().warn(TAG18, "Frame already exists, sra: " + this.useSra);
        this.getAdUrlDeferred.resolve("");
        return Promise.resolve("");
      }
      ...
      var identityPromise = timerService.timeoutPromise(1e3, this.identityTokenPromise_).catch(function() {
        return {};
      });
      var checkStillCurrent = this.verifyStillCurrent();
      ...
      var targetingExpansionPromise = timerService.timeoutPromise(1e3, this.expandJsonTargeting_(rtcParamsPromise)).catch(function() {
        dev().warn(TAG18, "JSON Targeting expansion failed/timed out.");
      });
      Promise.all([rtcParamsPromise, identityPromise, targetingExpansionPromise]).then(function(results) {
        ...
        googleAdUrl(_this5, DOUBLECLICK_BASE_URL, startTime, Object.assign(_this5.getBlockParameters_(), _this5.buildIdentityParams(), _this5.getPageParameters(consentTuple, void 0), rtcParams), _this5.experimentIds).then(function(adUrl) {
    ...
    _proto.expandJsonTargeting_ = function expandJsonTargeting_(rtcMergedPromise) {
      var _this6 = this;
    ...
    _proto.expandValue_ = function expandValue_(value) {
      var _this7 = this;
      if (!value) {
    ...
    _proto.expandString_ = function expandString_(string) {
      return Services.urlReplacementsForDoc(this.element).expandStringAsync(string, void 0, TARGETING_MACRO_ALLOWLIST);
    };
      ...
      rtcResponseArray.forEach(function(rtcResponse) {
        ...
        if (rtcResponse.response) {
          ...
          if (rtcResponse.response["ppid"]) {
            _this8.jsonTargeting["ppid"] = rtcResponse.response["ppid"];
          }
        }
      });
      if (exclusions) {
      ...
      return {
        ...
        ATTR: function ATTR(name) {
          if (!allowlist[name.toLowerCase()]) {
            dev().warn(TAG18, "Invalid attribute " + name);
          } else {
    ...
    _proto.onNetworkFailure = function onNetworkFailure(error, adUrl) {
      dev().info(TAG18, "network error, attempt adding of error parameter", error);
      return {
    ...
    _proto.getSlotSize = function getSlotSize() {
      var _this$getDeclaredSlot = this.getDeclaredSlotSize_(), height = _this$getDeclaredSlot.height, width = _this$getDeclaredSlot.width;
      return width && height ? {
        width: width,
        height: height
      } : this.getIntersectionElementLayoutBox();
      ...
      if (this.postAdResponseExperimentFeatures["render-idle-throttle"] && this.isIdleRender_) {
        if (is3pThrottled(this.win)) {
          return waitFor3pThrottle().then(function() {
            return _AmpA4A.prototype.renderNonAmpCreative.call(_this10);
          });
        } else {
    ...
    _proto.onCreativeRender = function onCreativeRender(creativeMetaData, opt_onLoadPromise) {
      ...
      if (this.isRefreshing) {
        devAssert2(this.refreshManager_);
        this.refreshManager_.initiateRefreshCycle();
        this.isRefreshing = false;
        this.isRelayoutNeededFlag = false;
      }
      var size = this.returnedSize_ || this.getSlotSize();
      var isMultiSizeFluid = this.isFluidRequest_ && this.returnedSize_ && !(size.width == 0 && size.height == 0);
      ...
      this.refreshManager_ = this.refreshManager_ || getRefreshManager(this, function() {
        if (_this11.useSra) {
          user().warn(TAG18, "Refresh not compatible with SRA.");
          return false;
        }
        if (getEnclosingContainerTypes(_this11.element).filter(function(container) {
          ...
          user().warn(TAG18, "Refresh not compatible with ad-containers, except for AMP-CAROUSEL and AMP-STICKY-AD");
          return false;
        }
        return true;
      });
    ...
    _proto.expandFluidCreative_ = function expandFluidCreative_() {
      var _this12 = this;
      if (this.isFluidRequest_ && !this.returnedSize_ && this.isVerifiedAmpCreative()) {
        if (!this.iframe || !this.iframe.contentWindow || !this.iframe.contentWindow.document || !this.iframe.contentWindow.document.body) {
          dev().error(TAG18, "Attempting to expand fluid creative without a properly set up friendly frame. Slot id: " + this.element.getAttribute("data-amp-slot-index"));
          return Promise.reject("Cannot access body of friendly frame");
        }
        return this.setCssPosition_("static").then(function() {
          return _this12.attemptChangeHeight(_this12.iframe.contentWindow.document.body.clientHeight).then(function() {
            _this12.fireFluidDelayedImpression();
            _this12.reattemptToExpandFluidCreative_ = false;
          }).catch(function() {
            user().warn(TAG18, "Attempt to change size failed on fluid creative. Will re-attempt when slot is out of the viewport.");
            var _this12$getSlotSize = _this12.getSlotSize(), height = _this12$getSlotSize.height, width = _this12$getSlotSize.width;
            if (width && height) {
              _this12.fireFluidDelayedImpression();
            }
    ...
    _proto.handleResize_ = function handleResize_(newWidth, newHeight) {
      ...
      var _this$getDeclaredSlot2 = this.getDeclaredSlotSize_(), height = _this$getDeclaredSlot2.height, width = _this$getDeclaredSlot2.width;
      ...
      if (isFluidRequestAndFixedResponse || returnedSizeDifferent && heightNotIncreased) {
        ...
        if (newWidth > width && (!this.isFluidPrimaryRequest_ || this.parameterSize && this.parameterSize.indexOf(newWidth + "x" + newHeight) == -1)) {
          this.adjustSlotPostExpansion_(newWidth);
        }
      }
    };
    _proto.adjustSlotPostExpansion_ = function adjustSlotPostExpansion_(newWidth) {
      if (!devAssert2(this.flexibleAdSlotData_, "Attempted to expand slot without flexible ad slot data.")) {
        return;
      }
      var _this$flexibleAdSlotD2 = this.flexibleAdSlotData_, parentStyle = _this$flexibleAdSlotD2.parentStyle, parentWidth = _this$flexibleAdSlotD2.parentWidth;
      var isRtl = isRTL(this.win.document);
      var dirStr = isRtl ? "Right" : "Left";
      var style = this.inZIndexHoldBack_ ? {
        "z-index": "11"
      } : {};
      if (parentStyle.textAlign != "center") {
        var getMarginStr = function getMarginStr2(marginNum) {
    ...
    _proto.sendXhrRequest = function sendXhrRequest(adUrl) {
      ...
      this.initiateSraRequests().then(function() {
        ...
        if (!_this14.sraDeferred) {
          dev().warn(TAG18, "SRA failed to include element " + _this14.ifi_);
      ...
      return this.sraDeferred.promise.then(function(response) {
        ...
        return response || _AmpA4A.prototype.sendXhrRequest.call(_this14, adUrl);
      });
    };
      ...
      impressions.split(",").forEach(function(url) {
        try {
          if (!Services.urlForDoc(_this15.element).isSecure(url)) {
            dev().warn(TAG18, "insecure impression url: " + url);
            return;
          }
          _this15.getAmpDoc().getBody().appendChild(createElementWithAttributes(_this15.win.document, "amp-pixel", dict({
            "src": url,
    ...
    _proto.initiateSraRequests = function initiateSraRequests() {
      ...
      sraRequests = sraRequests || this.groupSlotsForSra().then(function(groupIdToBlocksAry) {
        ...
        Object.keys(groupIdToBlocksAry).forEach(function(networkId) {
          var blocks = devAssert2(groupIdToBlocksAry[networkId]);
          sraRequestPromises.push(Promise.all(blocks).then(function(instances) {
            devAssert2(instances.length);
            checkStillCurrent();
            var typeInstances = instances.filter(function(instance) {
              var isValid = instance.hasAdPromise();
              if (!isValid) {
                dev().info(TAG18, "Ignoring instance without ad promise as likely invalid", instance.element);
              }
              return isValid;
            });
            ...
            if (!noFallbackExp && typeInstances.length == 1) {
              dev().info(TAG18, "single block in network " + networkId);
            ...
            return constructSRARequest_(_this16, typeInstances).then(function(sraUrlIn) {
              ...
              if (isCancellation(error)) {
                ...
                assignAdUrlToError(error, sraUrl);
    ...
    _proto.warnOnError = function warnOnError(message, error) {
      dev().warn(TAG18, message, error);
    };
    _proto.getRandomString_ = function getRandomString_() {
      var length = 16;
    ...
    _proto.getNonAmpCreativeRenderingMethod = function getNonAmpCreativeRenderingMethod(headerValue) {
      return this.forceSafeframe || this.isFluidRequest_ ? XORIGIN_MODE.SAFEFRAME : _AmpA4A.prototype.getNonAmpCreativeRenderingMethod.call(this, headerValue);
    };
    ...
    _proto.getAdditionalContextMetadata = function getAdditionalContextMetadata(isSafeFrame) {
      ...
      devAssert2(creativeSize, "this.getCreativeSize returned null");
      if (this.isRefreshing) {
        if (this.safeframeApi_) {
          this.safeframeApi_.destroy();
        }
        this.safeframeApi_ = new SafeframeHostApi(this, this.isFluidRequest_, creativeSize);
      } else {
        this.safeframeApi_ = this.safeframeApi_ || new SafeframeHostApi(this, this.isFluidRequest_, creativeSize);
      }
    ...
    _proto.postTroubleshootMessage = function postTroubleshootMessage() {
      ...
      devAssert2(this.troubleshootData_.adUrl, "ad URL does not exist yet");
      return this.troubleshootData_.adUrl.then(function(adUrl) {
        ...
        _this17.win.opener.postMessage(payload, "*");
      });
    };
    ...
    return AmpAdNetworkDoubleclickImpl2;
  }(AmpA4A);
  AMP.registerElement(TAG18, AmpAdNetworkDoubleclickImpl);
  ...
  function constructSRARequest_(a4a, instances) {
    devAssert2(instances && instances.length);
    var startTime = Date.now();
