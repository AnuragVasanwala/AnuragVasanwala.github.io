(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-viqeo-player",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  function experimentToggles(win) {
    ...
    if (isArray(allowedDocOptIn) && allowedDocOptIn.length) {
      ...
      if (meta) {
        ...
        for (var _iterator = _createForOfIteratorHelperLoose4(optedInExperiments), _step; !(_step = _iterator()).done; ) {
    ...
    if (isArray(allowedUrlOptIn) && allowedUrlOptIn.length) {
      ...
      for (var _iterator2 = _createForOfIteratorHelperLoose4(allowedUrlOptIn), _step2; !(_step2 = _iterator2()).done; ) {
  ...
  function getExperimentToggles(win) {
    ...
    var toggles = map();
    for (var _iterator3 = _createForOfIteratorHelperLoose4(tokens), _step3; !(_step3 = _iterator3()).done; ) {
  ...
  var AmpViqeoPlayer = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpViqeoPlayer2, _AMP$BaseElement);
    var _super = _createSuper(AmpViqeoPlayer2);
    function AmpViqeoPlayer2(element) {
      var _this;
    ...
    var _proto = AmpViqeoPlayer2.prototype;
    ...
    _proto.buildCallback = function buildCallback() {
      this.videoId_ = userAssert2(this.element.getAttribute("data-videoid"), "The data-videoid attribute is required for <amp-viqeo-player> %s", this.element);
      userAssert2(this.element.getAttribute("data-profileid"), "The data-profileid attribute is required for <amp-viqeo-player> %s", this.element);
      this.hasAutoplay_ = this.element.hasAttribute(VideoAttributes_Enum.AUTOPLAY);
      var deferred = new Deferred();
    ...
    _proto.layoutCallback = function layoutCallback() {
      var _this2 = this;
      var iframe = getIframe(this.win, this.element, "viqeoplayer", {
      ...
      this.unlistenMessage_ = listen(this.win, "message", this.handleViqeoMessages_.bind(this));
    ...
    _proto.handleViqeoMessages_ = function handleViqeoMessages_(event) {
      var eventData = getData(event);
      if (!eventData || event.source !== (this.iframe_ && this.iframe_.contentWindow) || eventData["source"] !== "ViqeoPlayer") {
        return;
      }
      ...
      if (action.startsWith("update")) {
        var key = action.replace(/^update([A-Z])(.*)$/, function(_, c, rest) {
    ...
    _proto.createPlaceholderCallback = function createPlaceholderCallback() {
      var placeholder = this.element.ownerDocument.createElement("img");
      ...
      placeholder.setAttribute("src", "https://cdn.viqeo.tv/preview/" + encodeURIComponent(this.videoId_) + ".jpg");
      return placeholder;
    };
    _proto.supportsPlatform = function supportsPlatform() {
      return true;
    ...
    _proto.getPlayedRanges = function getPlayedRanges() {
      return this.meta_["playedRanges"] || [];
    };
    ...
    return AmpViqeoPlayer2;
  }(AMP.BaseElement);
  AMP.registerElement(TAG4, AmpViqeoPlayer);
  var amp_viqeo_player_default = AmpViqeoPlayer;
})();
/*! https://mths.be/cssescape v1.5.1 by @mathias | MIT license */
})});
//# sourceMappingURL=amp-viqeo-player-0.1.max.js.map
