(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-jwplayer",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  var AmpJWPlayer = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpJWPlayer2, _AMP$BaseElement);
    var _super = _createSuper(AmpJWPlayer2);
    function AmpJWPlayer2(element) {
      var _this;
      _this = _super.call(this, element);
      _this.contentid_ = "";
      _this.playerid_ = "";
      _this.contentSearch_ = "";
      _this.contentRecency_ = "";
      _this.contentBackfill_ = "";
      _this.iframe_ = null;
      _this.playerReadyPromise_ = null;
      _this.playerReadyResolver_ = null;
      _this.onSetupOnce_ = once(function() {
        return _this.onSetup_();
      });
      _this.onReadyOnce_ = once(function(detail) {
        return _this.onReady_(detail);
      });
      _this.onMessage_ = _this.onMessage_.bind(_assertThisInitialized(_this));
      _this.playlistItem_ = null;
      _this.duration_ = 0;
      _this.currentTime_ = 0;
      _this.playedRanges_ = [];
      _this.unlistenFrame_ = null;
      _this.unlistenFullscreen_ = null;
      _this.pauseHelper_ = new PauseHelper(_this.element);
      _this.consentState_ = null;
      _this.consentString_ = null;
      _this.consentMetadata_ = null;
      return _this;
    }
    ...
    _proto.getMetadata = function getMetadata() {
      var playlistItem_ = this.playlistItem_, win = this.win;
    ...
    _proto.preconnectCallback = function preconnectCallback(onLayout) {
      ...
      var preconnectUrl = function preconnectUrl2(url) {
        return Services.preconnectFor(_this2.win).url(ampDoc, url, onLayout);
      };
    ...
    _proto.buildCallback = function buildCallback() {
      ...
      this.contentid_ = userAssert2(element.getAttribute("data-playlist-id") || element.getAttribute("data-media-id"), "Either the data-media-id or the data-playlist-id attributes must be specified for <amp-jwplayer> %s", element);
      this.playerid_ = userAssert2(element.getAttribute("data-player-id"), "The data-player-id attribute is required for <amp-jwplayer> %s", element);
    ...
    _proto.layoutCallback = function layoutCallback() {
      var _this3 = this;
      return this.getConsentData_().then(function() {
        var _this3$consentMetadat;
        var queryParams = dict({
          "search": _this3.getContextualVal_() || void 0,
          "recency": _this3.contentRecency_ || void 0,
          "backfill": _this3.contentBackfill_ || void 0,
          "isAMP": true,
          "consentState": _this3.consentState_ || void 0,
          "consentValue": _this3.consentString_ || void 0,
          "consentGdpr": ((_this3$consentMetadat = _this3.consentMetadata_) == null ? void 0 : _this3$consentMetadat.gdprApplies) || void 0
        });
        var url = _this3.getSingleLineEmbed_();
        var src = addParamsToUrl(url, queryParams);
        src = addParamsToUrl(src, getDataParamsFromAttributes(_this3.element, null, /^playerParam(.+)/));
        if (_this3.queryString_) {
          src += "&" + _this3.queryString_;
        }
        var frame = disableScrollingOnIframe(createFrameFor(_this3, src, _this3.element.id));
        addUnsafeAllowAutoplay(frame);
        disableScrollingOnIframe(frame);
        _this3.unlistenFrame_ = listen(_this3.win, "message", _this3.onMessage_);
        _this3.unlistenFullscreen_ = listen(frame, "fullscreenchange", function() {
          var isFullscreen = _this3.isFullscreen();
          _this3.sendCommand_("setFullscreen", isFullscreen);
        });
        _this3.iframe_ = frame;
        return _this3.loadPromise(_this3.iframe_);
      });
    };
    ...
    _proto.onSetup_ = function onSetup_() {
      var element = this.element;
      var configAttributes = getDataParamsFromAttributes(element, null, /^config(.+)/);
      var configJSON = element.getAttribute("data-config-json");
      var config = tryParseJson(configJSON) || {};
      Object.keys(configAttributes).forEach(function(attr) {
        if (attr.indexOf("json") !== -1) {
          return;
        }
        config[attr] = configAttributes[attr];
      });
      var adCustParamsJSON = element.getAttribute("data-ad-cust-params");
      if (adCustParamsJSON) {
        config.adCustParams = tryParseJson(adCustParamsJSON);
      }
      var adMacros = getDataParamsFromAttributes(element, null, /^adMacro(.+)/);
      if (Object.keys(adMacros).length !== 0) {
        config.adMacros = adMacros;
      }
      this.postCommandMessage_("setupConfig", config);
    };
    _proto.onReady_ = function onReady_(detail) {
      var element = this.element;
      this.playlistItem_ = _extends({}, detail.playlistItem);
    ...
    _proto.onMessage_ = function onMessage_(messageEvent) {
      if (!this.iframe_ || !messageEvent || messageEvent.source != this.iframe_.contentWindow) {
        return;
      }
      var messageData = getData(messageEvent);
      ...
      if (detail && event) {
        switch (event) {
          case "fullscreen":
            var fullscreen = detail.fullscreen;
            if (fullscreen !== this.isFullscreen()) {
              fullscreen ? this.fullscreenEnter() : this.fullscreenExit();
            }
            break;
          case "meta":
            var duration = detail.duration, metadataType = detail.metadataType;
            if (metadataType === "media") {
              this.duration_ = duration;
            }
            break;
          case "mute":
            var mute = detail.mute;
            var _element = this.element;
            dispatchCustomEvent(_element, mutedOrUnmutedEvent(mute));
            break;
          case "playedRanges":
            ...
            break;
          case "playlistItem":
            var playlistItem = _extends({}, detail);
    ...
    _proto.sendCommand_ = function sendCommand_(method, optParams) {
      var _this4 = this;
      this.playerReadyPromise_.then(function() {
        return _this4.postCommandMessage_(method, optParams);
      });
    };
    _proto.postCommandMessage_ = function postCommandMessage_(method, optParams) {
    ...
    _proto.isSafariOrIos_ = function isSafariOrIos_() {
      var platform = Services.platformFor(this.win);
      return platform.isSafari() || platform.isIos();
    };
    _proto.getSingleLineEmbed_ = function getSingleLineEmbed_() {
      var isDev = getMode(this.win).localDev;
      var pid = encodeURIComponent(this.playerid_);
      var cid = encodeURIComponent(this.contentid_);
      if (cid === "outstream") {
        cid = "oi7pAMI1";
      }
      var baseUrl = "https://content.jwplatform.com/players/" + cid + "-" + pid + ".html";
      if (isDev) {
        var testPage = new URLSearchParams(document.location.search).get("test_page");
        if (testPage) {
    ...
    _proto.getContextualVal_ = function getContextualVal_() {
      if (this.contentSearch_ === "__CONTEXTUAL__") {
        var context = this.getAmpDoc().getHeadNode();
        var ogTitleElement = context.querySelector('meta[property="og:title"]');
        var ogTitle = ogTitleElement ? ogTitleElement.getAttribute("content") : null;
        var title = (context.querySelector("title") || {}).textContent;
        return ogTitle || title || "";
      }
      return this.contentSearch_;
    };
    _proto.getConsentData_ = function getConsentData_() {
      var _this5 = this;
      var consentPolicy = _AMP$BaseElement.prototype.getConsentPolicy.call(this);
      var consentStatePromise = consentPolicy ? getConsentPolicyState(this.element, consentPolicy) : Promise.resolve(null);
      var consentStringPromise = consentPolicy ? getConsentPolicyInfo(this.element, consentPolicy) : Promise.resolve(null);
      var consentMetadataPromise = consentPolicy ? getConsentMetadata(this.element, consentPolicy) : Promise.resolve(null);
      return Promise.all([consentStatePromise, consentStringPromise, consentMetadataPromise]).then(function(consents) {
        _this5.consentState_ = consents[0];
        _this5.consentString_ = consents[1];
        _this5.consentMetadata_ = consents[2];
      });
    };
    return AmpJWPlayer2;
  }(AMP.BaseElement);
