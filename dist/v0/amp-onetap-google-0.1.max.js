(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-onetap-google",ev:"0.1",l:true,f:(function(AMP,_){
(function() {
  ...
  // build/amp-onetap-google-0.1.css.js
  var CSS2 = ".i-amphtml-onetap-google-iframe{z-index:2147483647;border:none;position:fixed}.i-amphtml-onetap-google-ui-card{width:391px;right:20px;top:20px}.i-amphtml-onetap-google-ui-bottom_sheet{right:auto;left:0;top:auto;bottom:0;width:100%}\n/*# sourceURL=/extensions/amp-onetap-google/0.1/amp-onetap-google.css*/";
  ...
  // extensions/amp-onetap-google/0.1/amp-onetap-google.js
  ...
  var TAG = "amp-onetap-google";
  var SENTINEL = "onetap_google";
  var ACTIONS = {
  ...
  var AmpOnetapGoogle = /* @__PURE__ */ function(_AMP$BaseElement) {
    _inherits(AmpOnetapGoogle2, _AMP$BaseElement);
    var _super = _createSuper(AmpOnetapGoogle2);
    function AmpOnetapGoogle2(element) {
      var _this;
      _this = _super.call(this, element);
      _this.iframe_ = null;
      _this.shouldCancelOnTapOutside_ = true;
      _this.unlisteners_ = null;
      return _this;
    }
    var _proto = AmpOnetapGoogle2.prototype;
    ...
    _proto.buildCallback = function buildCallback() {
      ...
      this.getAmpDoc().whenFirstVisible().then(function() {
        _this2.createIframe_(assertHttpsUrl(_this2.element.dataset.src, _this2.element));
      });
    };
    _proto.postMessage_ = function postMessage_(source, message, origin) {
      source.postMessage(message, origin);
    };
    _proto.handleIntermediateIframeMessage_ = function handleIntermediateIframeMessage_(origin, event) {
      var _this3 = this;
      if (!this.iframe_) {
        return;
      }
      ...
      switch (data["command"]) {
        case ACTIONS.READY:
          ...
          this.postMessage_(event.source, dict({
            "sentinel": SENTINEL,
        ...
        case ACTIONS.RESIZE:
          ...
          if (typeof height === "number" && !isNaN(height) && height > 0) {
            this.mutateElement(function() {
              toggle(_this3.element, height > 0);
              setStyle(_this3.iframe_, "height", px(data["height"]));
            });
          }
          break;
        case ACTIONS.CLOSE:
          this.mutateElement(function() {
            _this3.removeIframe_();
          });
          break;
        case ACTIONS.DONE:
          this.mutateElement(function() {
            _this3.removeIframe_();
            _this3.refreshAccess_();
          });
          break;
        case ACTIONS.SET_UI_MODE:
          var uiMode = data["mode"];
          if (!uiMode) {
            return;
          }
    ...
    _proto.setUiMode_ = function setUiMode_(mode9) {
      var _this4 = this;
      this.mutateElement(function() {
        _this4.iframe_.classList.add("i-amphtml-onetap-google-ui-" + mode9);
      });
    };
    _proto.refreshAccess_ = function refreshAccess_() {
      Promise.all([this.refreshAmpAccess_(), this.refreshAmpSubscriptions_()]).then(function(refreshed) {
        if (!refreshed.reduce(function(a, b) {
          return a || b;
        })) {
          user().warn(TAG, "Sign-in was completed, but there were no entitlements to refresh. Please include amp-access or amp-subscriptions.");
        }
      });
    };
    _proto.refreshAmpAccess_ = function refreshAmpAccess_() {
      var accessElement = this.getAmpDoc().getElementById("amp-access");
      if (!accessElement) {
        return false;
      }
      Services.actionServiceForDoc(this.element).execute(accessElement, "refresh", null, null, null, null, ActionTrust_Enum.DEFAULT);
      return true;
    };
    _proto.refreshAmpSubscriptions_ = function refreshAmpSubscriptions_() {
      return Services.subscriptionsServiceForDocOrNull(this.element).then(function(subscriptions) {
        if (!subscriptions) {
          return false;
        }
    ...
    _proto.createIframe_ = function createIframe_(srcUnexpanded) {
      var _this5 = this;
      if (this.iframe_) {
        return;
      }
      this.iframe_ = this.getAmpDoc().win.document.createElement("iframe");
      Services.urlReplacementsForDoc(this.element).expandUrlAsync(srcUnexpanded).then(function(srcExpanded) {
        if (!_this5.win) {
          return;
        }
        _this5.insertIframe_(srcExpanded);
      });
    };
    _proto.insertIframe_ = function insertIframe_(src) {
      var _this6 = this;
      devAssert2(this.iframe_);
      toggle(this.element, false);
      var _Services$urlForDoc$p = Services.urlForDoc(this.element).parse(src), origin = _Services$urlForDoc$p.origin;
      this.unlisteners_ = [listen(this.win, "message", function(event) {
        _this6.handleIntermediateIframeMessage_(origin, event);
      }), listen(this.getAmpDoc().getRootNode(), "click", function() {
        if (_this6.shouldCancelOnTapOutside_ && !_this6.element.hasAttribute("hidden")) {
          _this6.removeIframe_();
        }
      })];
    ...
    return AmpOnetapGoogle2;
  }(AMP.BaseElement);
  AMP.registerElement(TAG, AmpOnetapGoogle, CSS2);
})();
/*! https://mths.be/cssescape v1.5.1 by @mathias | MIT license */
})});
//# sourceMappingURL=amp-onetap-google-0.1.max.js.map
