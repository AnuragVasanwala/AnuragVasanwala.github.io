(self.AMP=self.AMP||[]).push({m:0,v:"2109221842260",n:"amp-iframe",ev:"1.0",l:false,f:(function(AMP,_){
(function() {
  ...
  // extensions/amp-iframe/1.0/component.js
  var _excluded2 = ["allowFullScreen", "allowPaymentRequest", "iframeStyle", "onLoad", "referrerPolicy", "requestResize", "sandbox", "src", "srcdoc"];
  function _extends3() {
  ...
  function BentoIframe(_ref) {
    var allowFullScreen = _ref.allowFullScreen, allowPaymentRequest = _ref.allowPaymentRequest, iframeStyle = _ref.iframeStyle, _ref$onLoad = _ref.onLoad, onLoad = _ref$onLoad === void 0 ? NOOP : _ref$onLoad, referrerPolicy = _ref.referrerPolicy, requestResize = _ref.requestResize, sandbox = _ref.sandbox, src = _ref.src, srcdoc = _ref.srcdoc, rest = _objectWithoutPropertiesLoose4(_ref, _excluded2);
    var iframeRef = useRef();
    var dataRef = useRef(null);
    var isIntersectingRef = useRef(null);
    var containerRef = useRef(null);
    var observerRef = useRef(null);
    var targetOriginRef = useRef(null);
    var viewabilityCb = function viewabilityCb2(entries) {
      var iframe = iframeRef.current;
      var targetOrigin = targetOriginRef.current;
      if (!iframe || !targetOrigin) {
        return;
      }
      postMessage(iframe, MessageType_Enum.INTERSECTION, dict({
        "changes": entries.map(cloneEntryForCrossOrigin)
      }), targetOrigin);
    };
    var handleSendIntersectionsPostMessage = useCallback(function(event) {
      var _event$data;
      var iframe = iframeRef.current;
      if (!iframe) {
        return;
      }
      if (event.source !== iframe.contentWindow || ((_event$data = event.data) == null ? void 0 : _event$data.type) !== MessageType_Enum.SEND_INTERSECTIONS) {
        return;
      }
      targetOriginRef.current = event.origin;
      var win = getWin(iframe);
    ...
    useEffect(function() {
      ...
      return function() {
        var _observer;
        (_observer = observer) == null ? void 0 : _observer.unobserve(iframe);
        observer = null;
        win.removeEventListener("message", handleSendIntersectionsPostMessage);
      };
    }, [handleSendIntersectionsPostMessage]);
    var updateContainerSize = function updateContainerSize2(height, width) {
    ...
    var attemptResize = useCallback(function() {
      var _dataRef$current, _dataRef$current2;
      var height = Number((_dataRef$current = dataRef.current) == null ? void 0 : _dataRef$current.height);
      var width = Number((_dataRef$current2 = dataRef.current) == null ? void 0 : _dataRef$current2.width);
      dataRef.current = null;
      if (!height && !width) {
        console.error("Ignoring resize request because width and height value is invalid");
        return;
      }
      var iframe = iframeRef.current;
      if (!height) {
    ...
    var handleEmbedSizePostMessage = useCallback(function(event) {
      var _event$data2;
      if (((_event$data2 = event.data) == null ? void 0 : _event$data2.type) !== MessageType_Enum.EMBED_SIZE) {
        return;
      }
    ...
    var ioCallback2 = useCallback(function(_ref2) {
    ...
    var measureRef = useIntersectionObserver(ioCallback2);
    ...
    }, [src, srcdoc, sandbox, allowFullScreen, allowPaymentRequest, referrerPolicy, onLoad]);
    return createElement(ContainWrapper, _extends3({
      contentAs: "iframe",
      contentProps: contentProps,
      contentRef: useMergeRefs([iframeRef, measureRef]),
      contentStyle: _extends3({
  ...
  function _setPrototypeOf(o3, p2) {
    _setPrototypeOf = Object.setPrototypeOf || function _setPrototypeOf5(o4, p3) {
      o4.__proto__ = p3;
      return o4;
    };
  ...
  function _setPrototypeOf2(o3, p2) {
    _setPrototypeOf2 = Object.setPrototypeOf || function _setPrototypeOf5(o4, p3) {
      o4.__proto__ = p3;
      return o4;
    };
  ...
  // extensions/amp-iframe/1.0/base-element.js
  ...
  BaseElement2["Component"] = BentoIframe;
  BaseElement2["props"] = {
    "src": {
      attr: "src"
    },
  ...
  // extensions/amp-iframe/1.0/amp-iframe.js
  ...
  var TAG2 = "amp-iframe";
  var MINIMUM_DISTANCE_FROM_TOP_PX = 600;
  var MINIMUM_VIEWPORT_PROPORTION = 0.75;
  var AmpIframe = /* @__PURE__ */ function(_BaseElement) {
    _inherits4(AmpIframe2, _BaseElement);
    var _super = _createSuper4(AmpIframe2);
    function AmpIframe2() {
      return _super.apply(this, arguments);
    }
    var _proto = AmpIframe2.prototype;
    _proto.isLayoutSupported = function isLayoutSupported(layout) {
      userAssert(isExperimentOn(this.win, "bento") || isExperimentOn(this.win, "bento-iframe"), 'expected global "bento" or specific "bento-iframe" experiment to be enabled');
      return _BaseElement.prototype.isLayoutSupported.call(this, layout);
    };
    _proto.handleOnLoad = function handleOnLoad() {
      var _this = this;
      ...
      measureIntersection(this.element).then(function(intersectionEntry) {
        var top = intersectionEntry.boundingClientRect.top;
        var viewportHeight = intersectionEntry.rootBounds.height;
        var minTop = Math.min(MINIMUM_DISTANCE_FROM_TOP_PX, viewportHeight * MINIMUM_VIEWPORT_PROPORTION);
        userAssert(top >= minTop, "<amp-iframe> elements must be positioned outside the first 75% of the viewport or 600px from the top (whichever is smaller): %s  Current position: %s. Min: %sPositioning rules don't apply for iframes that use `placeholder`.See https://github.com/ampproject/amphtml/blob/main/extensions/amp-iframe/amp-iframe.md#iframe-with-placeholder for details.", _this.element, top, minTop);
      });
    };
    _proto.updateSize_ = function updateSize_(height, width) {
      if (height < 100) {
        this.user().error(TAG2, "Ignoring embed-size request because the resize height is less than 100px. If you are using amp-iframe to display ads, consider using amp-ad instead.", this.element);
    ...
    _proto.init = function init() {
      var _this2 = this;
      return dict({
        "requestResize": function requestResize(height, width) {
          return _this2.updateSize_(height, width);
        }
      });
    };
    return AmpIframe2;
  }(BaseElement2);
  AMP.registerElement(TAG2, AmpIframe);
})();
...
//# sourceMappingURL=amp-iframe-1.0.max.js.map
