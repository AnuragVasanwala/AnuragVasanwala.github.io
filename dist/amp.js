...
(function() {
  ...
  function handleClickUrl(win) {
    var ampdoc2 = Services.ampdoc(win.document.documentElement);
    var viewer = Services.viewerForDoc(ampdoc2);
  ...
  function preconnectToOrigin(document) {
    return whenDocumentComplete(document).then(function() {
      ...
      if (win) {
        ...
        var ampdoc2 = Services.ampdoc(document);
        preconnect.url(ampdoc2, info.sourceUrl);
        preconnect.url(ampdoc2, info.canonicalUrl);
      }
    });
  }
  ...
  function installAutoLightboxExtension(ampdoc2) {
    var win = ampdoc2.win;
    if (!isAmphtml(win.document) || !ampdoc2.isSingleDoc()) {
      return;
    }
    chunk(ampdoc2, function() {
      isStoryDocument(ampdoc2).then(function(isStory) {
        if (isStory) {
          return;
        }
        Services.extensionsFor(win).installExtensionForDoc(ampdoc2, "amp-auto-lightbox");
      });
  ...
  // src/standalone.js
  function installStandaloneExtension(ampdoc2) {
    var win = ampdoc2.win;
    if (!isAmphtml(win.document)) {
      return;
    }
    if (!Services.platformFor(ampdoc2.win).isStandalone()) {
      return;
    }
    chunk(ampdoc2, function() {
      Services.extensionsFor(win).installExtensionForDoc(ampdoc2, "amp-standalone").then(function() {
        return Services.standaloneServiceForDoc(ampdoc2.getBody());
  ...
  // src/amp.js
  function bootstrap(ampdoc2, perf) {
    startupChunk(self.document, function services() {
      installRuntimeServices(self);
      installAmpdocServices(ampdoc2);
    ...
    startupChunk(self.document, function final() {
      ...
      installAutoLightboxExtension(ampdoc2);
      installStandaloneExtension(ampdoc2);
      maybeValidate(self);
  ...
  startupChunk(self.document, function initial() {
    ...
    if (isEsm()) {
      bootstrap(ampdoc2, perf);
      return;
    }
    installStylesForDoc(ampdoc2, cssText + cssText2, function() {
      return bootstrap(ampdoc2, perf);
    }, true, "amp-runtime");
  });
  if (self.console) {
